...
test("the original Phase 2 baseline stays immutable while Phase 5 waves through #154 have exact reviewed closures", async () => {
  ...
  assert.equal(manifest.contract.reviewedWaves[19].id, "phase5-issue154-v1");
  assert.equal(manifest.contract.reviewedWaves[19].issue, 154);
  assert.deepEqual(manifest.contract.reviewedWaves[19].commands, []);
  assert.deepEqual(manifest.contract.reviewedWaves[19].imports, ["github.com/vegastack/vegastack-labs/internal/adapter/backuptrust"]);
