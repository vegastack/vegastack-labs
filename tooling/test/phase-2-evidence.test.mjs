...
test("the original Phase 2 baseline stays immutable while Phase 5 backup verification has its own exact reviewed wave", async () => {
  ...
  assert.equal(manifest.contract.reviewedWaves[10].id, "phase5-issue117-v1");
  assert.equal(manifest.contract.reviewedWaves[10].issue, 117);
  assert.deepEqual(manifest.contract.reviewedWaves[10].commands, ["backup run", "backup status", "backup verify"]);
