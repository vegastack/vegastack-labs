...
test("the original Phase 2 baseline stays immutable while Phase 5 waves through #117 integrated after #153 have exact reviewed closures", async () => {
  ...
  assert.equal(manifest.contract.reviewedWaves?.length, 16);
  ...
  assert.equal(manifest.contract.reviewedWaves[15].id, "phase5-issue117-v1");
  assert.equal(manifest.contract.reviewedWaves[15].issue, 117);
  assert.deepEqual(manifest.contract.reviewedWaves[15].commands, ["backup run", "backup status", "backup verify"]);
  assert.deepEqual(manifest.contract.reviewedWaves[15].imports, []);
  assert.equal(facts.postPhase2MutationBoundaryDigest, manifest.contract.reviewedWaves[15].mutationBoundaryDigest);
