...
test("the original Phase 2 baseline stays immutable while #104, #123, #128, #124, #107, and #106 have exact reviewed waves", async () => {
  ...
  assert.equal(manifest.contract.reviewedWaves?.length, 6);
  ...
  // #107 audit read wave lands last, carrying the combined head-closure digest.
  ...
  // #106 local backup-creation wave lands last, carrying the combined head-closure digest.
  assert.equal(manifest.contract.reviewedWaves[5].id, "phase5-issue106-v1");
  assert.equal(manifest.contract.reviewedWaves[5].issue, 106);
  assert.deepEqual(manifest.contract.reviewedWaves[5].commands, ["backup policy draft"]);
  ...
  assert.equal(facts.postPhase2MutationBoundaryDigest, manifest.contract.reviewedWaves[5].mutationBoundaryDigest);
