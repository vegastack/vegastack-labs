...
test("the original Phase 2 baseline stays immutable while Phase 5 waves through #140 and #153 have exact reviewed closures", async () => {
  ...
  assert.equal(manifest.contract.reviewedWaves[12].id, "phase5-issue153-v1");
  assert.deepEqual(manifest.contract.reviewedWaves[12].commands, ["recovery witness collect"]);
  assert.deepEqual(manifest.contract.reviewedWaves[12].imports, ["github.com/vegastack/vegastack-labs/internal/adapter/recoverydenial"]);
