...
test("the original Phase 2 baseline stays immutable while Phase 5 waves through #154 have exact reviewed closures", async () => {
  ...
  assert.equal(manifest.contract.reviewedWaves?.length, 21);
  ...
  assert.equal(facts.postPhase2MutationBoundaryDigest, manifest.contract.reviewedWaves[20].mutationBoundaryDigest);
