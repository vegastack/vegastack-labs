...
test("the original Phase 2 baseline stays immutable while #104, #123, #128, #124, #107, #125, #132/#133/#134, #106, #146 and #153 have exact reviewed waves", async () => {
  ...
  assert.equal(manifest.contract.reviewedWaves?.length, 12);
  ...
  assert.equal(manifest.contract.reviewedWaves[11].id, "phase5-issue153-v1");
  assert.deepEqual(manifest.contract.reviewedWaves[11].commands, ["recovery witness collect"]);
  assert.deepEqual(manifest.contract.reviewedWaves[11].imports, ["github.com/vegastack/vegastack-labs/internal/adapter/recoverydenial"]);
  assert.equal(facts.postPhase2MutationBoundaryDigest, manifest.contract.reviewedWaves[11].mutationBoundaryDigest);
