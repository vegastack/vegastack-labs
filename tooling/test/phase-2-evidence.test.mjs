...
test("the original Phase 2 baseline stays immutable while #104, #123, #128, #124, #107, #125, and #132, and #106 have exact reviewed waves", async () => {
  ...
  assert.equal(manifest.contract.reviewedWaves?.length, 8);
  ...
  // #106 local backup-creation wave is the current exact closure.
  assert.equal(manifest.contract.reviewedWaves[7].id, "phase5-issue106-v1");
  assert.equal(manifest.contract.reviewedWaves[7].issue, 106);
  assert.deepEqual(manifest.contract.reviewedWaves[7].commands, ["backup policy draft"]);
  ...
  assert.equal(facts.postPhase2MutationBoundaryDigest, manifest.contract.reviewedWaves[7].mutationBoundaryDigest);
