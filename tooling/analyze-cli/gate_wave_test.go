...
func TestReviewedCredentialImportAndAuditLocalClientWavesRejectChangedAndAddedSource(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"linux", []string{"audit_client.go", "backup_client.go", "client.go", "credential_client.go", "credential_lifecycle_client.go", "gates_client.go", "listener.go", "listener_linux.go", "restore_client.go", "schedule_client.go"}},
		{"unsupported", []string{"audit_client.go", "backup_client.go", "client.go", "credential_client.go", "credential_lifecycle_client.go", "gates_client.go", "listener.go", "listener_unsupported.go", "restore_client.go", "schedule_client.go"}},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			...
			for _, clientFile := range []string{"audit_client.go", "credential_client.go", "credential_lifecycle_client.go", "restore_client.go", "schedule_client.go"} {
