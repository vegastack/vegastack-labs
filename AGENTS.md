...
During implementation and review fixes, run only the narrow unit, schema, fixture, security, failure, recovery, and integration checks affected by the current change. During the Phase 5 delivery batch, run one successful exact-base/exact-head affected public check at the clean branch head immediately before asking the operator to create an intermediate issue's pull request. Changed Go packages receive focused tests and vet while repository-wide compilation and affected boundary checks remain. Run the complete public suite once for the final Phase 5 integration or acceptance candidate, selected explicitly. Do not repeat a selected lane on an unchanged head. If review or conflict-resolution changes that head afterward, rerun only the affected checks; the pull-request candidate is verified again by CI.
