...
test("a new run starts SSE without the previous run cursor", async ({ page }) => {
  changeFixture.eventMode = "reconnect";
  changeFixture.switchRunIdAfterFirstEvent = true;
  await page.goto("/changes");
  await page.getByLabel("Declaration ID").fill("declaration-one");
  await page.getByLabel("Revision").fill("1");
  await page.getByRole("button", { name: "Open declaration" }).click();
  ...
  await page.getByRole("button", { name: "Start run" }).click();
  await page.getByRole("button", { name: "Start exact run" }).click();
  await expect.poll(() => changeFixture.eventConnections).toBe(1);
  await expect(page.locator('[data-run-status="interrupted"]')).toBeVisible();
  await page.getByRole("button", { name: "Start run" }).click();
  await page.getByRole("button", { name: "Start exact run" }).click();
  await expect.poll(() => changeFixture.eventConnections).toBeGreaterThanOrEqual(2);
  expect(changeFixture.activeRunId).toBe("run-two");
  expect(changeFixture.eventLastIds.slice(0, 2)).toEqual(["", ""]);
  expect(changeFixture.executeRequests).toBe(2);
});
