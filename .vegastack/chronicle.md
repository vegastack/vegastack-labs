...
- **How it went:** The security boundary stayed inside one executable; adversarial JWT, key-outage, cookie, replay, audit, recovery, and authorization tests drove the implementation. Linux race tests exposed replay-idempotency and renewal foreign-key ordering defects that macOS skips, and both were fixed. The reviewed Issue #51/#52 contract chain was then integrated, the combined additive contract moved to 1.8.0, and generated browser reads remained mutation-free.
- **Changed:** Provider-neutral verified identities · strict RS256 Access JWT validation · lazy key refresh with a 24-hour known-key outage bound · 15-minute idle/8-hour absolute digest-only sessions · exact Host/Origin/JWT/session ordering · secure local-only logout · audited revocation and Cloudflare-independent local recovery.
