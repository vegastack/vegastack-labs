...
- **What:** Development now has one shared check catalog and a deterministic Git-diff selector. Local work runs only affected checks, while one complete clean-head check remains required before a pull request; PR and `main` CI install Chromium only when browser-facing code changed.
...
- **Changed:** Shared ordered check groups · fail-closed changed-path plan · conditional Chromium CI · one-full-run-before-PR rule in every agent mandate.
