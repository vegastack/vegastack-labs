...
- **How it went:** The original two-field plan command lacked the server-owned observation facts needed for an exact plan request, and a disconnected submit did not yet know which run to inspect. A narrow authorized preparation read and one shared deterministic run-ID helper closed those gaps without adding a client state machine. Independent review then caught client-inferred recovery advice, incomplete unknown-outcome guidance and process coverage, and a source-regex network guard. The correction moved run presentation into a generated server-owned view, added typed inspect-only uncertainty, exercised the complete protected-socket outcome matrix, and replaced the regex with typed Go dependency analysis plus a sealed, value-only HTTP-over-Unix transport. `localapi` can no longer import or mutate HTTP clients, transports, requests, URLs, dialers, callbacks, connections, or alternate network syscalls; only its fully reviewed source may import the transport, and any transport or caller-source/API change now fails closed for re-review. Focused race tests and a Debian built-process fixture proved exact request bytes, hostile path handling, one submit, one recovery read, and no resubmission.
- **Changed:** Generated available plan/apply/run commands · exact plan-preparation read · server-owned durable run presentation · thin Unix-socket client methods · deterministic durable run IDs · typed disconnect inspection without retry · human/JSON parity · stable server error exits · protected-socket outcome matrix · sealed value-only local transport · portable target builds and typed CLI boundary guards.
