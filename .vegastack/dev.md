...
- guard: before asking for a pull request, run one successful selected public check at the exact clean local and pushed branch head. Intermediate issues supply the exact current `main` SHA as `base_sha` and use the affected plan; the final Phase 5 integration/acceptance candidate omits `base_sha` and explicitly selects `full_check`. The installed dev-ship PR guard verifies exact base/head, structured plan and successful run/job/step result without repeating the suite. Missing or unverifiable proof blocks rather than launching an implicit duplicate.
