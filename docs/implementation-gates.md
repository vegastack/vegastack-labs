...
The implemented dormant credential foundation does not change this gate's state. Its synthetic exact-ID SDK, systemd host-key and run-order fixtures are software tests, not vault-reader-set, custody, provider-denial, recovery or activation evidence. Production still has no registered credential resolver or authoritative live-proof verifier, and the import endpoint remains planned. `G-007` therefore stays evidence-required and continues to block every provider credential activation until the secrets administrator supplies the real positive and negative provider tests and recovery evidence above.
