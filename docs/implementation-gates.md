...
The implemented dormant credential foundation and local inert import do not change this gate's state. The available import endpoint accepts only local protected input and creates an append-only draft plus host-key ciphertext; it does not register a production resolver, supply an authoritative live-proof verifier or activate a credential. Its synthetic exact-ID SDK, systemd host-key, restart, denial and import fixtures are software tests, not vault-reader-set, custody, provider-denial, independent recovery or activation evidence. A successful fixture or import cannot pass or close `G-007`. The gate therefore stays evidence-required and continues to block every provider credential activation until the secrets administrator supplies the real positive and negative provider tests and recovery evidence above.

The recovery-source package and qualification code of #159 likewise leaves its production direct-denial registry empty. Administrator enrollment of the independent source, exact live endpoint qualification and #108 replacement authority remain separate prerequisites; its disposable process proof cannot close `G-007` or admit credential recovery.
