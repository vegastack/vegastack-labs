...
Issue #117 adds the exact local verifier: a pending point can become local last-good only after point-bound object hashing, pinned full `check --read-data`, isolated SQLite restore, current revision/epoch/key/dependency checks and a non-overdue policy v1.2 cadence. A v1.1 policy or fixture, failed, stale, uncertain or overdue attempt cannot qualify. The generated status/run/verify API and CLI route through the same human-approved plan executor. The current protected verifier proves only the pinned binary and SQLite schema catalog; config/image/signature dependencies fail closed without a trusted resolver. Even a local verified point does not close `G-008`, which still requires real off-site lock, independent key, media and clean-host evidence.
