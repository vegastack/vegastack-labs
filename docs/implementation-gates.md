...
The implemented local read commands are `vsk-labs gate list --config <profile>`, `gate inspect --config <profile> --gate-id <id>`, and `gate check --config <profile> --gate-id <id> --subject-id <id>`; `--output json` forwards the exact generated server result. Their applicability/reason/source/ready-for-input fields come from generated definitions and the server-owned applied profile/policy scope, not a CLI assertion. A missing or stale applied scope remains `unknown`; a fixture or an unregistered live proof verifier cannot pass. Current read/check projections have no authoritative server-owned subject declaration/revision/artifact resolver. They preserve specific fail-closed fixture, expiry, replacement and missing-evidence blockers, but never expose a live `passed` result; a potentially positive or unbound revision result becomes `unknown`/`subject-binding-unavailable`. A later production collector must supply that binding before these reads may derive a live `passed` result; caller-provided target digests are not a substitute.

The operator-only `vsk-labs gate profile draft --config <profile> --file <exact-json>` accepts a bounded 4 KiB generated profile/policy candidate with the current state revision, recovery epoch and canonical target digest. It persists only an inert draft and exact declaration. The applied binding changes only after `gate.profile.bind` executes through a current exact human-approved Phase 4 plan; there is no direct profile setter or new approval branch. `vsk-labs gate evidence --config <profile> --file <exact-json>` similarly accepts a bounded 64 KiB typed request, authorizes it and creates an inert draft/declaration. Public evidence input cannot set its own source/proof class or include raw/private attachment content; current public submissions are fixture-class until a production verifier/collector is separately implemented. The tested, restricted digest-addressed attachment-store primitive is dormant internal preparation: no production caller can write or read it, and its existence is not proof that attachment intake or live collection is enforced. A later separately reviewed collector must wire it before attached proof is accepted. Supersession/revocation is append-only via a new exact plan, never a status edit or delete.
...
The operator-only `vsk-labs backup policy draft --config <profile> --file <exact-json>` follows the same inert pattern for `G-008`'s creation input: it validates and stores a bounded canonical backup policy, returns an opaque draft ID and digest, and binds that digest through one `x-backup-policy` declaration extension. Applying it runs solely through the exact human-approved Phase 4 plan/run path, which captures the declared source consistently and creates one encrypted restic-v2 **pending** local recovery point (see [Backup and recovery](security-and-operations.md#backup-and-recovery)). A pending point is fixture-class creation evidence only: an exit-0 restic run or a green metadata check does not qualify it, and it can never close `G-008`, advance local last-good, or stand as live off-site or clean-host-restore proof. `G-008` still requires the pinned binary digest, repository IDs, capacity forecast, R2 lock proof, independent key recovery and functional clean-host restores, which #117/#114 and their live evidence own.
...
Issue #117 adds the exact local verifier: a pending point can become local last-good only after point-bound object hashing, pinned full `check --read-data`, isolated SQLite restore, current revision/epoch/key/dependency checks and a non-overdue policy v1.2 cadence. A v1.1 policy or fixture, failed, stale, uncertain or overdue attempt cannot qualify. The generated status/run/verify API and CLI route through the same human-approved plan executor. The protected verifier proves the pinned binary and SQLite schema catalog. The signed-dependency resolver can additionally verify `config`, `image` and `signature` artifacts against one current revisioned source/root and bind the exact point, policy, revision and epoch, but production has no registered site artifact reader or source/root registration surface. Those kinds therefore remain blocked outside synthetic fixtures. Even a local verified point does not close `G-008`, which still requires real off-site lock, independent key, media and clean-host evidence.
...
The complete native lifecycle software does not change this gate's activation state. Import, stage, native restart/positive/denied verification, bounded-overlap rotation, explicit revocation and clean-host recovery have an integrated disposable-Linux acceptance path. Those results prove the public software and fail-closed boundaries; they are not actual vault/item identities, service-account grants, reader-set tests, enrolled recovery custody or live provider observations. The optional 1Password resolver remains absent from the production registry. A successful fixture, import or native lifecycle run cannot pass or close `G-007`. The gate stays `evidence-required` and blocks provider credential activation until the secrets administrator supplies the exact positive, negative and recovery evidence above.

The installed recovery-source package likewise leaves its production direct-denial registry empty. Administrator enrollment of the independent source, exact live endpoint qualification and current replacement authority remain separate prerequisites; disposable process and lifecycle proofs cannot close `G-007` or admit a live credential recovery.

### Backup engine and repository topology — `G-008`

The software topology uses one independent encrypted R2 generation per locally verified critical point. This avoids the shared-repository failure where an older deduplicated pack can lose protection while a newer retained point still needs it. Five generation-qualified payload prefixes are protected indefinitely, `locks/` remains mutable, and the short-lived writer cannot administer rules. The actual destination must still qualify those lock, credential, cutoff, multipart, capacity and clean-host recovery semantics under `G-008`; no fixture result is activation evidence. [Pinned restic lock behavior](https://raw.githubusercontent.com/restic/restic/v0.19.1/internal/restic/lock.go)

Current signed-dependency software does not change this gate's activation state. A real site must separately approve and install the protected artifact reader, register the exact artifact/bundle/root digests plus signer identity and issuer through an inert revisioned declaration and human-approved plan, and then produce point-bound evidence at the current state revision and recovery epoch. Wrong, revoked, stale, missing or forged trust records fail closed with a sanitized reason and preserve the earlier last-good. Fixture roots and signed test artifacts cannot close `G-008`; direct file paths, caller-supplied root bytes and SQLite edits are not registration or recovery procedures.
...
Control-authority restore software does not close `G-008`. Before a site attempt, the gate bundle must identify the selected current local last-good point or qualified off-site generation, matching release/tool/schema and retained dependencies, independent checkpoint or explicitly accepted lost interval, recovery-key availability, and fresh direct-denial evidence for every applicable former-controller boundary. The generated restore plan must bind those facts, the fixed target set, candidate identity, new instance ID and exactly adjacent epoch. Acceptance preserves the former database, proves the promoted database starts read-only, rejects every prior-epoch authority artifact, completes the full canary including a new verified backup, and proves a returning former controller cannot mutate. Missing real media, key, independent fence, clean replacement host or negative probe keeps `G-008` evidence-required and the database `recovery-required`.

The off-site retirement software path similarly proves only the closed adapter contract: exact five-rule subtraction through a full rule-configuration PUT, complete reread before any listed-key delete, distinct JIT identities, write-ahead per-effect records, uncertainty on response loss, and full survivor recovery proof. Fake R2 race and denial fixtures cannot qualify exclusive one-owner administration, actual Cloudflare rule behavior, real credentials or live recovery. Those remain required `G-008` evidence, and live retirement stays unavailable until they are current.
...
- `critical-offsite`: one independent encrypted R2 S3-compatible generation for each verified critical point; indefinite rules protect its retained payload prefixes, `locks/` remains mutable, and no `forget` or `prune` runs inside the retained generation.
- Off-site writers cannot delete/overwrite protected payloads or change bucket-lock policy. They receive bounded `ListBucket`, `PutObject`, `GetObject` and mutable-lock `DeleteObject` authority for one generation through the one-run IAM endpoint. Rule administration and whole-generation retirement remain separate human-approved authority. Real R2 must prove the exact action/path and post-expiry denial behavior.
...
- Before activating a fixed scheduled policy, verify its exact human approval plan, effective policy grant, current state revision and recovery epoch, target/source/subject digest, credential references, retention digest, maximum work, cadence/window/retry bounds and expiry. Each occurrence repeats those checks and the action-specific current proof. Missing or stale proof blocks the occurrence; uncertain effect state forbids automatic retry.
