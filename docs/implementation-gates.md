...
|---|---|---|---|---|---|---|
| `G-001` | Serial/node-map integrity | design-closed | evidence-required | lead infrastructure operator | console/label photos or signed physical worksheet for `PF4F10C1` and `PG025CNM`; one-to-one serial/node uniqueness; approved Sheet correction and credential-hygiene attestation | blocks phase 1 and every hostname/role apply |
...
| `G-006` | CI runner controller and admission | design-closed | evidence-required | CI adapter owner | explicit enrolled repository/workflow list; runner group/label proof; hostile-job isolation, one-job lifecycle, cancellation cleanup and external-log preservation tests | blocks phase-4 runner enrollment |
| `G-007` | 1Password topology and recovery | design-closed | evidence-required | secrets administrator | actual vault/item IDs, scoped service-account grants, offline recovery custody receipt, positive reads and all required negative reads | blocks phase-3 provider credential activation |
| `G-008` | Backup repositories and recovery | design-closed | evidence-required | backup administrator | pinned restic binary digest, SSD/R2 repository IDs, capacity forecast, R2 lock proof, independent key recovery and functional clean-host restores | blocks phase-3 control acceptance |
...
| `G-012` | Tunnel connector placement | design-closed | evidence-required | access-adapter owner | qualification of default candidates `vsk-node-02` and `vsk-node-08`, route/credential separation and single-connector failure test; substitute only by plan | blocks phase-5 Tunnel activation |
| `G-013` | LAN managed-network marker | design-closed | evidence-required | access-adapter owner | qualified default host `vsk-node-02`, pinned TLS certificate fingerprint/expiry/rotation owner and safe remote-profile fallback test | blocks phase-2 managed-network activation |
| `G-014` | Coolify initial release | design-closed | evidence-required | platform administrator | revalidate selected `v4.3.10`, tag commit and installer digest; clean install/backup/restore/rollback proof on the qualified control host | blocks phase-3 bootstrap |
...
| `G-017` | Platform release identity and feeds | design-closed | evidence-required | platform release owner | exact repository/workflow identity, feed URLs, OIDC issuer/identity policy and successful offline bundle verification for every asset | blocks first platform release and phase-3 install |
...
- ARM64 self-hosted macOS remains public preview as revalidated on 2026-08-25. Fallback is GitHub-hosted `macos-15-intel` for compatible build/test work plus a protected manual Mac-mini lane for Apple-silicon-only validation; no preview lane is silently treated as mandatory. [GitHub self-hosted runners](https://docs.github.com/en/actions/reference/runners/self-hosted-runners) · [GitHub monitoring](https://docs.github.com/en/actions/how-tos/manage-runners/self-hosted-runners/monitor-and-troubleshoot) · [GitHub-hosted runners](https://docs.github.com/en/actions/reference/runners/github-hosted-runners) [D-111](decisions-and-sources.md#d-111)

### 1Password layout — `G-007`

The selected deployment layout uses separate vaults so no service account becomes a universal credential:

| Logical vault | Contents | Machine access |
|---|---|---|
| `VegaStack Labs Control Plane` | server session/signing references, database/audit operational identities | control service account; read only |
| `VegaStack Labs Cloudflare` | read and mutation-token items separated by field/item | Cloudflare observation or mutation service account, never both unless the approved API lacks separation |
| `VegaStack Labs Coolify` | dashboard-read, platform-admin and per-team deploy-token source items | corresponding adapter/rotation identity only |
| `VegaStack Labs Registry` | Harbor administrator recovery, robots and TLS references | registry rotation/backup identities only; CI receives only its delivered robot secret |
| `VegaStack Labs Backup` | restic repository passwords, SSD/R2 object credentials and signing/checkpoint references | separate backup writer and retention administrator identities |
| `VegaStack Labs Projects — <slug>` | project-specific deploy/notification source values | that project's rotation identity; runtime values are delivered to the target secret store |
| `VegaStack Labs Break Glass` | sealed recovery and account-recovery material | human owners only; no service account |

Each item uses a stable logical ID, owner, consumers, created/rotated/expiry dates and provider resource reference. Automated retrieval uses `op read` or `op run` with a service account restricted to named vaults; `RESTIC_PASSWORD_COMMAND` is used for restic so the repository password is not placed in a long-lived environment variable. Actual vault/item UUIDs never enter portable config. [1Password secret scripts](https://developer.1password.com/docs/cli/secrets-scripts/) [D-110](decisions-and-sources.md#d-110)
...
- Backup writers can create/read repository objects but cannot change bucket-lock policy. A separately administered retention identity is used only for reviewed `forget`/`prune`; data writers never hold it. R2's bucket lock remains an independent protection because restic itself needs delete access for pruning.
