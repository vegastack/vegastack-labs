...
The portable client profile is a strict `vegastack-labs.dev/client-profile` `1.0.0` document whose transport is `constrained-ssh`, executable is `ssh` (or `ssh.exe`), destination is one bounded `user@host`, and `knownHostsPath` is an absolute platform path. The client constructs the fixed options `BatchMode=yes`, `ClearAllForwardings=yes`, `ExitOnForwardFailure=yes`, `StrictHostKeyChecking=yes`, and `UserKnownHostsFile=<knownHostsPath>`, then supplies only the destination. It sends no remote command: the server-side forced command binds the authenticated SSH identity and carries the existing local API request/response bytes under the framing guard. Paths and metacharacters remain literal argument values; no shell parses them.
