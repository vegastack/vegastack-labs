...
### Phase 5 generated contract boundary

Issue [5.1 (#102)](https://github.com/vegastack/vegastack-labs/issues/102) extends the existing [metadata graph](../internal/metadata/phase5.go) and [contract generator](../internal/contractgen/generate.go). The same graph emits [gate definitions](../schemas/v1/gate-definition.schema.json), evidence/evaluations, credential references and redacted resolution records, backup/recovery points, audit checkpoints, restore bindings, exact-job policies, Go types/validators, CLI/API metadata and the [browser-safe client](../web/generated/read-api.ts). The checked-in generator drift check is the public contract authority; generated files are not edited independently.

These Phase 5 commands and endpoints remain `planned`. A generated client method is a typed seam, not an implemented API handler, qualified evidence, a grant or permission to run infrastructure. Evidence requests cannot claim their own source or proof class; `fixture` proof is never live qualification, and the later evaluator must refuse expired proof. `not-applicable` is not `passed`. Credential values and reveal paths are absent from public shapes; resolution records have no browser audience. Browser restore reads contain status only, not acknowledgement or former-controller fence proof.

Restore records bind one retained point and dependencies, exact targets and digest, the Phase 4 plan ID/digest, responsible human acknowledgement, prior/new controller identities, former-controller fence and an incremented recovery epoch. Exact scheduled jobs bind policy revision, action and target digests; they are not cron, a new worker or unattended approval. Generated validators reject malformed shape, unknown major/state, fixture-to-live relabeling, invalid lifecycle edges and binding widening, but they do not persist evidence, resolve secrets, verify a live backup, fence a controller or run a job. Those behaviors and isolated recovery proofs belong to the later approved Phase 5 issues and still require the existing plan/policy authority.
...
Issue #66 defines the public contract but does not make `plan`, `apply`, or any Phase 4 endpoint runnable. They remain `planned` until their owning engine issues implement and test the routes. The generated provider-neutral schemas are closed authoritative inputs: unknown fields, missing bindings, unknown states, secret-shaped names, and every version other than the declared exact version fail. A read-only compatibility decoder may ignore documented safe top-level additions in the same major and recursively rejects secret-shaped keys inside their values; known nested bindings and embedded contracts remain exact. Compatible reads cannot authorize, persist, execute, lease, receipt, or calculate a canonical digest.
...
Issue #69 makes only the generated executor claim, renew, and receipt endpoints available. They use the same `server run` process and protected remote listener; there is no executor daemon, queue, webhook, or provider-specific control path. A machine request must carry the already configured external identity assertion, resolve to a declared principal, omit browser Origin/session credentials, and match the claim's `principalId`; the lifecycle service then reauthorizes that principal as the exact plan-declared executor. The server selects the work and rechecks every exact binding. A successful claim returns one 60-second lease; renewal is due at 20 seconds, rotates only the permitted nonce state, and cannot change a target, artifact, adapter, executor, recovery epoch, or maximum expiry.

If the executor misses renewal, it must stop starting new steps. Expiry does not prove an outside effect stopped, failed, or succeeded, so the server marks the result ambiguous and requires reconciliation before another claim. Submitted progress and terminal receipts are observations only: independent adapter verification decides whether the step is verified, partial, or recovery-required. The account-free public simulator covers claim, renewal, connection loss, reconciliation, and receipt tampering with synthetic provider-neutral data and no network access. The human fallback inspects the durable run and target through the same API, performs the same independent verification, and records reconciliation; it never reassigns the step, edits SQLite, or accepts a receipt as proof.

Issue #78 exposes this Phase 4 lifecycle through the portable operator CLI. `plan` accepts one exact inert declaration revision, performs no confirmation and returns the immutable server plan. `apply` accepts one exact plan ID and cannot create or stand in for a Slack acknowledgement. `run inspect`, `run cancel`, and `run resume` name one durable run; the server, not the client, decides state transitions, safe cancellation boundaries, resumability, verification and recovery requirements. Human output and the single-line JSON envelope report the same plan digest, targets, risk, acknowledgement requirement, run state, completed and incomplete steps, verification result and next safe action. JSON mode never prompts; its one envelope is stdout, while human diagnostics remain stderr.

The CLI uses the reviewed local/constrained client boundary only. It does not open SQLite, call a provider, dispatch a shell, or accept an arbitrary HTTP destination. Request bodies bind the server-returned plan/run state and use opaque idempotency keys; retrying a transport operation with a different body is forbidden. `plan` first reads a typed server-owned preparation containing the current state revision, recovery epoch and observation fingerprint; plan creation revalidates all three. Before `apply` submits, the client holds one stable idempotency key and derives the same run ID as the server through their shared provider-neutral helper. If the execute transport disconnects after the server may have accepted it, the client performs exactly one `GET /api/v1/runs/{runId}` and never submits `apply` again. If that durable read is unavailable, the client reports an unknown transport outcome and directs the operator to the same explicit `run inspect --run-id <id>` operation; it never invents execution state.

A separately declared class covers repetitive, non-destructive operational jobs such as consistent backup creation, integrity verification, provider observation refresh and retention-safe audit export. `schedule policy draft` stores the exact inert policy; a human-authorized `schedule.policy.activate` plan is the only activation path. The policy binds its approving human and plan, declaration revision, grant revision, state revision, recovery epoch, fixed action, adapter, sources, subjects, targets, credentials, maximum work, retention digest, cadence, window, retry bounds and expiry.

The OS timer invokes `schedule dispatch` over the protected local API. The server derives the due slot, rejects caller time drift, stale policy/state/epoch/grant or changed targets, creates a fresh 30-minute immutable preauthorized plan within the occurrence window and submits it through the shared run engine without an acknowledgement. Duplicate wakeups resolve to the same `(policy, revision, scheduledAt)` occurrence. Only a failure proven before every effect may retry; an intent, receipt, transport ambiguity or unknown effect becomes `uncertain` and requires operator reconciliation. Restore, retention change, pruning/deletion and production-like deployment are excluded from this action set. `backup run` remains the human-invoked exact-plan path. [D-081](decisions-and-sources.md#d-081)

The closed v1 action catalog binds gate checks and declaration observations to the read-only `core.schedule-observe` effect, local creation and integrity verification to the real `local.backup` adapter, and checkpoint export to `core.audit`. Scheduled backup credentials use a separate immutable plan-bound manifest so the adapter's policy/manifest digest and the secret binding digest stay independently sealed. The ordinary credential path remains human-authorized; the scheduled exception exists only with both schedule extensions, the configured local runner principal, a live occurrence lease, and repeated scheduled admission. Audit scheduling stays blocked unless signer, encrypted exporter, independent reader, public verification material, and namespace are all composed.

The human fallback uses the same `schedule dispatch --policy-id …` command and local profile as the timer. Inspect the returned job and linked run; never invoke an adapter directly. A blocked or skipped occurrence is resolved by correcting the named prerequisite or authoring a new policy revision. An uncertain occurrence is inspected through the durable run record and reconciled before another occurrence is admitted.
...
The portable client profile is a strict `vegastack-labs.dev/client-profile` `1.0.0` document whose transport is `constrained-ssh`, executable is an absolute protected platform path to `ssh` (or `ssh.exe`), destination is one bounded `user@host`, and `knownHostsPath` is an absolute platform path using only literal OpenSSH-safe path characters—no token expansion, whitespace, quotes or escapes. It also declares the expected `sshPrincipalId`, `deviceId`, and current `recoveryEpoch`; these values are not identity evidence, and the forced-command server checks them against the authenticated server-owned binding and current control state. The profile, executable and pinned host-key file plus their directory chains must have stable identity and protected ownership, permissions and link state; Windows additionally requires protected access-control lists and rejects reparse points. The client never resolves the executable through ambient `PATH`, ignores ambient SSH configuration and constructs a fixed noninteractive invocation that disables agents, password/keyboard authentication, proxy/jump commands, local/remote commands, forwarding, multiplexing, host-key updates and alternate global known-host files while requiring `StrictHostKeyChecking=yes` and the exact protected `UserKnownHostsFile`. It supplies only the destination and sends no client-selected remote command. The SSH account's server-owned forced command invokes the same `vsk-labs server api-ssh` executable entry with the verified principal, device and protected server profile; that entry accepts one bounded frame and forwards only its generated allowlisted API operation to the protected local socket. Values outside OpenSSH's config grammar remain literal argument values; no shell parses them.
