...
The portable client profile is a strict `vegastack-labs.dev/client-profile` `1.0.0` document whose transport is `constrained-ssh`, executable is an absolute protected platform path to `ssh` (or `ssh.exe`), destination is one bounded `user@host`, and `knownHostsPath` is an absolute platform path. It also declares the expected `sshPrincipalId`, `deviceId`, and current `recoveryEpoch`; these values are not identity evidence, and the forced-command server checks them against the authenticated server-owned binding and current control state. The profile, executable and pinned host-key file must be protected regular files with stable identity, ownership, permissions and link count; Windows additionally requires a protected owner and access-control list. The client never resolves the executable through ambient `PATH`, ignores ambient SSH configuration and constructs a fixed noninteractive invocation that disables agents, password/keyboard authentication, proxy/jump commands, local/remote commands, forwarding, multiplexing, host-key updates and alternate global known-host files while requiring `StrictHostKeyChecking=yes` and the exact protected `UserKnownHostsFile`. It supplies only the destination and sends no client-selected remote command. The SSH account's server-owned forced command invokes the same `vsk-labs server api-ssh` executable entry with the verified principal, device and protected server profile; that entry accepts one bounded frame and forwards only its generated allowlisted API operation to the protected local socket. Paths and metacharacters remain literal argument values; no shell parses them.
