...
The ES216G's DHCP capability is a client for its own management IP, not a DHCP server for the fleet. Keep the actual router as DHCP authority. Its Easy Managed classification does not promise SNMP/CLI, ACLs or routed interfaces. The [network-device capability and operations plan](network-device-operations.md) maps the official features, standalone/controller differences, guided UI ownership and firmware/recovery limits to implementation phases. No Omada controller or new DHCP service is introduced.
...
Inspect the primary and secondary HX510 modes and the observed DHCP server first. Do not enable DHCP on a satellite merely because it is nearer the switch. The guide does not establish whether the ACT firmware supports outside-pool reservations; the [firmware-dependent reservation question](network-device-operations.md#address-allocation-and-first-setup) remains pending. The current outside-dynamic-pool policy below is unchanged until explicitly revised.
...
### First address discovery and assignment

The ES216G connects the wired devices; the router remains the DHCP authority. The switch needs one address for its own management interface, while every machine has its own address. Switch port numbers and node numbers do not automatically determine IP addresses.

For the later authorized setup, use this order:

1. Confirm the delivered ES216G model, regional hardware revision and matching manual. Connect one router uplink and the operator workstation using the declared port map; do not add a second uplink or adopt an Omada controller.
2. On the actual lab LAN, inspect the workstation's IPv4 address, subnet mask and default gateway. In the router's local UI, inspect the DHCP pool, reservations and current leases. Do not infer these facts from a workstation connected elsewhere, or assume the ISP firmware exposes every vendor feature.
3. Locate the switch's DHCP lease by matching its MAC label; open that address from the same subnet and verify the device identity. TP-Link documents this discovery route. If no lease exists, inspect cabling and the matching manual's discovery/recovery procedure; do not guess a factory IP, reset a configured device or change the router subnet. [Official IP-discovery procedure](https://support.omadanetworks.com/uk/document/107618/)
4. Prepare a proposed address map from the verified subnet, usable host range, DHCP behavior, existing reservations/static devices and physical inventory. Preserve the gateway and other occupied addresses. An unanswered ping is not proof that an address is free. Reserve a contiguous infrastructure block excluded from ordinary dynamic allocation; if the actual firmware cannot express that policy, stop and resolve the existing fallback before applying changes.
5. Bind the switch's management MAC and each node's wired MAC to their approved addresses. An infrastructure admin performs the declared router/switch UI steps; `vsk-labs` must present their prerequisites, exact target values, verification and recovery rather than pretend an unimplemented adapter performed them. Apply one change at a time and prove reconnection, identity and persistence after the relevant lease renewal/reboot. Do not remove the current management path until its replacement works.
6. Retain the private mapping as `node -> verified serial -> wired MAC -> reserved LAN IP -> switch port -> role`; add Mesh addresses only after enrollment. Before the server exists, the approved bootstrap contract must protect this evidence locally and import it once into the control database. That contract is a [phase 0 deliverable](development/phases/00-development-foundation.md#solution-and-readiness-work), not permission to create a Git-backed inventory or alternate controller.

Actual addresses remain unassigned by this specification. Cabling, recovery access, protected administrator credentials and exact UI limitations are site inputs. No switch/router adapter, broad network scan or live configuration is authorized by this procedure.
...
The TLS marker is a minimal HTTPS endpoint on the LAN with a pinned certificate/SPKI fingerprint and no secret response. The marker host is one qualified application node selected from `vsk-node-03`, `vsk-node-05` and `vsk-node-07` by a placement plan; the access-adapter owner owns certificate rotation. `G-013` requires that host's qualification, actual fingerprint/expiry and safe remote-profile fallback before phase 2; a planned substitute is allowed. Marker loss intentionally selects remote Mesh. `vsk-labs connect` records only the chosen path/reason and timing, never credentials. Diagnosis order is: validate target alias and SSH host-key mapping; inspect marker result; probe reserved LAN address without changing routes; inspect One Client registration/profile/Mesh IP; probe Mesh address; then report the first failed invariant. Operators can force `--path lan` or `--path mesh` for diagnosis, but only direct IP plus pinned host key is the break-glass recovery path. A wrong host key, public resolution or dual identity mismatch is a hard stop, not an automatic fallback. [Implementation gates](implementation-gates.md#provider-ownership--g-012-through-g-016-g-019)

If future requirements need system-wide private DNS, treat that as a separate reviewed design. As revalidated 24-08-2026, Cloudflare Mesh hostname routes require MASQUE and Linux client version `2026.6.822.0` or later, and private DNS topology has additional return-route constraints. [Cloudflare Mesh routes](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-mesh/routes/)

## Public and protected ingress

`cloudflared` establishes outbound-only connections; no HX510 port forwarding is allowed. Use one named tunnel with at least two connectors on different qualified application nodes. Select two qualified application nodes from `vsk-node-03`, `vsk-node-05` and `vsk-node-07` by an explicit placement plan; `G-012` admits them only after burn-in, a one-connector-loss test and evidence that they do not share one disk or power adapter. A failed candidate is replaced through an epoch-bound placement plan, not by silently weakening redundancy. [Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/) [D-045](decisions-and-sources.md#d-045)

The control database owns tunnel name, connector node IDs, hostname-to-origin declarations, exposure class, Access audience and health check; Cloudflare owns connector/runtime state. The direct typed Cloudflare adapter owns declared VegaStack Labs Cloudflare objects; v1 has no Terraform/OpenTofu state. Bootstrap only after local HTTPS origin health passes: create or explicitly import the owned tunnel, deliver the declared tunnel-scoped runtime credential through 1Password into separately protected storage on each connector, install a pinned `cloudflared` package on connector A, verify origin and Access allow/deny, then repeat for B. Never place a tunnel credential in Compose, the database, a plan or a shared node image. An object created outside the adapter is observation-only until a reviewed import plan establishes single ownership. [D-112](decisions-and-sources.md#d-112)

Routine upgrade drains and updates one connector, proves the other remains healthy, then reverses. Replicas of the same tunnel do not have independently revocable tunnel credentials. The topology-preserving candidate uses a remotely managed tunnel; a locally managed import cannot reuse this token procedure without its own reviewed compatibility path. The candidate uses tunnel-wide token rotation with staged replica updates/restarts; old tokens cannot start new connections, but existing connections persist until disconnected. Approved compromise containment must rotate the token and terminate all old active connections, which may interrupt ingress while local/SSH administration remains. Shared-credential blast radius and that outage behavior require user approval in the phase-7 solution; absent approval, this capability stays blocked. Do not silently add separate tunnels, paid load balancing or weaker Access/origin rules. Test ordinary failover, staged reconnection, denial of an old-token connector and termination of old sessions. [Cloudflare tunnel token lifecycle](https://developers.cloudflare.com/tunnel/advanced/tunnel-tokens/) Diagnosis distinguishes origin failure, Access-policy denial, DNS, connector disconnect and Cloudflare edge failure using sanitized connector/HTTP evidence. Rollback removes the new hostname/policy or restores the previous pinned connector/config while keeping the private origin reachable by LAN/Mesh. Public failure never changes an exposure class to less restrictive or opens a router port.
...
|---|---|
| Bootstrap Coolify on existing control | Start from the already initialized, qualified control host and retain its vsk-labs identity, SQLite, credentials and audit. Verify applicable inventory/LAN/secret/recovery prerequisites and installer effects; retrieve the official installer through a reviewed artifact/hash path; install the selected Coolify version; bind management to protected/private paths; configure backup and least-privilege teams; verify login, database, queue, local Docker, a disposable resource and unchanged vsk-labs access/history. Failed installation/postchecks preserve the controller and evidence, stop dependent work and follow a scoped cleanup/recovery plan. Clean OS preparation belongs to an earlier separately approved adoption/reimage; full-host rebuild requires its own destructive plan, independent backup and fencing, never automatic failure compensation. |
...
CI admission must validate the actual job assigned to the runner before any workflow checkout, user-controlled step or deploy-secret delivery. Approving the queued trigger alone is insufficient with shared labels. Use tested provider routing restrictions plus a trusted, non-job-writable actual-job admission guard; failure, cancellation, identity mismatch or inability to verify refuses execution. Qualification must race allowed and denied jobs sharing labels and prove zero user steps/secrets for denied assignments.
...
- Give the registry declared HTTPS endpoints using the ordinary application exposure/routing contract. The public stable name may use Tunnel; private LAN/Mesh clients use an explicitly declared and TLS-verified private application endpoint when required. Do not assume SSH/CLI alias resolution affects Docker/BuildKit/browser DNS. Exact names and supported client routing are qualified with ordinary app permissions; do not add special Harbor host/client privileges, a general private DNS service or automatic OS/Docker reconfiguration. If a same-name LAN/private/public arrangement cannot be proven within those bounds, return an explicit endpoint choice before activation. Use a trusted origin certificate and never run the registry over plaintext HTTP. [Harbor HTTPS](https://goharbor.io/docs/main/install-config/configure-https/)
...
Harbor remains an ordinary Coolify-managed application. Its lifecycle adapter/consistency hook uses only declared application permissions, not additional fleet/control authority. Unsupported app-local administration is an assigned app-owner manual step with verification. Required recovery-artifact retention protects digests/signatures needed by retained deployments and backups from garbage collection.
