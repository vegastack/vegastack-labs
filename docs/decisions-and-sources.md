...
| <a id="d-125"></a> [D-125](#d-125) | Remote browser authentication remains inside the one `vsk-labs` Go executable: a provider-neutral verified-identity adapter validates the VegaStack Labs Cloudflare Access JWT, while Go-owned SQLite stores only digested bindings and sessions. This is the documented exception to the general Better Auth default because an added Node authentication runtime would create a second runtime and authority path. Signing keys refresh at startup and for an unknown `kid`; an outage permits only a known key for at most 24 hours since the last successful fetch. Local sessions expire after 15 minutes idle or 8 hours absolute, rotate on renewal and are invalidated by logout, revocation, grant or recovery-epoch change. Logout is local only, and protected local OS-peer recovery remains independent. | Direct user selections for issue #54 on 10-09-2026; SRC-CF-13; SRC-JOSE-01. |
