...
| <a id="d-109"></a> [D-109](#d-109) | The VegaStack Labs deployment profile selects restic repository format v2 with separate encrypted local standard and critical repositories. Each verified critical local recovery point uses one independent encrypted off-site R2 generation. Indefinite rules protect that generation’s `config`, `keys/`, `data/`, `index/` and `snapshots/`; `locks/` remains mutable; a short-lived generation-scoped writer cannot administer rules; and only a separate human-approved exact retirement plan may remove a whole generation. No `forget` or `prune` runs inside a retained off-site generation. | direct user instruction 25-08-2026; operator-delegated D-109 correction recorded on issue #114 on 22-09-2026; #103 compatibility evidence; SRC-BKP-01..SRC-BKP-03; SRC-R2-02 |
...
| <a id="d-122"></a> [D-122](#d-122) | V1 uses Slack as the only normal human-acknowledgement adapter for bootstrap and mutation. Socket Mode runs inside `vsk-labs server run`; app-level `connections:write` and bot `chat:write` are the minimum declared scopes. A verified configured workspace/user action binds the exact plan or signed installation manifest, local human, targets, reason, risk, expiry/nonce, state revision and recovery epoch; Slack is neither operational-state, audit nor execution authority. Private approver mappings are inert desired state and contain no token/signing secret. Missing/unavailable Slack blocks normal bootstrap/apply. Account-free human approval is deferred beyond v1 as an accepted limitation; separately authorized break-glass recovery is not a routine fallback. | Direct user selection for issue #16 on 04-09-2026; SRC-SL-01..SRC-SL-05. |
| <a id="d-123"></a> [D-123](#d-123) | Host Ansible connects through a dedicated non-root automation identity and receives no broad become grant. Its only elevation is a root-owned, non-resident one-shot mode of the single `vsk-labs` executable that independently verifies the immutable plan/action bundle, digests, host identity, declaration revision, recovery epoch, approved action set, expiry and recovery precheck; it rejects arbitrary modules, commands and target widening, retains no desired state and exits after the exact action. | Direct user selection for issue #17 on 04-09-2026; SRC-ANS-02. |
| <a id="d-124"></a> [D-124](#d-124) | Automated macOS management uses constrained SSH through Cloudflare Mesh only. Default-deny network policy binds declared identity, device posture, destination and SSH port; Remote Login binds declared users. The native application firewall stays enabled but is not a CIDR filter. Product-managed PF, MDM, TCC/SIP bypass, default remote Full Disk Access and silent direct-LAN fallback are excluded. Supported local consent and physical-console recovery remain human prerequisites. | Direct user selection for issue #17 on 04-09-2026; SRC-CF-12; SRC-MAC-04..SRC-MAC-07. |
| <a id="d-125"></a> [D-125](#d-125) | Remote browser authentication remains inside the one `vsk-labs` Go executable: a provider-neutral verified-identity adapter validates the VegaStack Labs Cloudflare Access JWT, while Go-owned SQLite stores only digested bindings and sessions. This is the documented exception to the general Better Auth default because an added Node authentication runtime would create a second runtime and authority path. Signing keys refresh at startup and for an unknown `kid`; concurrent misses against one cache generation coalesce, and an outage permits only a known key for at most 24 hours since the last successful fetch. Session POSTs require the exact Origin. A safe same-origin GET/HEAD whose browser omits Origin requires the exact browser-controlled `same-origin`/`cors`/`empty` Fetch Metadata tuple instead; any present mismatch or incomplete substitute fails closed. Local sessions expire after 15 minutes idle or 8 hours absolute, rotate on renewal and are invalidated by logout, revocation, grant or recovery-epoch change. Time expiry transitions durably and once; repeated principal revocation is scoped to the latest append-only session generation and proves no active target remains. Logout is local only, and protected local OS-peer recovery remains independent. | Direct user selections for issue #54 on 10-09-2026; review correction on 10-09-2026; SRC-CF-13; SRC-FETCH-01; SRC-JOSE-01. |

[Portable lifecycle](platform-lifecycle.md) is the current generic contract. D-118 supersedes mandatory external-workstation bootstrap and universal physical-serial/provider assumptions. D-122 supersedes only D-118's claim that human-approved bootstrap/apply is account-free; D-118's selected-host setup, account-free read/preparation, local authentication, credential resolution and recovery responsibilities remain. D-123 closes the Ansible privilege mechanism left open by D-116, and D-124 closes D-119's Mac source-filter/consent mechanism without adding MDM or PF. D-119 supersedes old connector/marker defaults on 02/08. D-120 supersedes any unconditional claim that restic/R2 no-delete writer compatibility is closed. Original historical decisions/source evidence remain readable; the current requirements and qualified applicability determine new plans.
...
| backup, restore and audit | SQLite declarations/run ledger; selected backup manifests and independent audit checkpoints; Labs restic/R2 uses the selected independent-generation topology | backup/audit owner distinct from workload; recovery-key custodian | account-free independent recovery path; Labs G-008 provider, credential, retained-dependency and clean-host evidence before actual repositories, keys, R2 locks/capacity and consistency qualification | consistent snapshot → verify → restic encrypt into one independent generation → verify preconfigured lock coverage → upload → check → functional restore drill | fixed schedules pre-authorized; no restore/delete/retention change without plan; data/export writer cannot change bucket lock | policy change/restore/cutover by infra admin; scheduled fixed run needs no per-run ack | manifest/hash/object/lock, weekly/monthly checks and quarterly functional restore | backup age, source adapter, repository, key, capacity, audit chain/checkpoint | earlier verified point, isolated clean-node restore, signed snapshot last resort | selected D-109/D-120 topology; actual G-008 repository/binary/rule/action/cutoff/key/capacity and clean-host proof remains; development phase 5, deployment phase 3 acceptance |
...
|---|---|---|
| Material design/qualification open | G-006 actual-job admission; G-007 physical vault reader-set layout; G-012 tunnel credential/containment approval | Resolve the named solution before its implementation issue is ready, then obtain actual provider/site evidence. A gate alone does not solve the design. |
| Selected design; activation qualification open | G-008 independent off-site generation topology and retained-payload compatibility are selected under D-109/D-120; actual provider, credential, repository, rule/action/cutoff, capacity, key and clean-host recovery evidence remains | The software design is closed; only exact provider/site activation evidence can pass the gate. |
...
| SRC-CF-12 | [Network policies](https://developers.cloudflare.com/cloudflare-one/traffic-policies/network-policies/) and [Gateway identity selectors](https://developers.cloudflare.com/cloudflare-one/traffic-policies/identity-selectors/) | default-deny non-HTTP/SSH policy by identity, device posture, traffic source, destination and port; checked 04-09-2026 |
| SRC-CF-13 | [Validate an Access JWT](https://developers.cloudflare.com/cloudflare-one/access-controls/applications/http-apps/authorization-cookie/validating-json/) | Access assertion, issuer/audience/signature/time validation and public-key rotation boundary; checked 10-09-2026 |
...
| SRC-FETCH-01 | [WHATWG Fetch — Origin header](https://fetch.spec.whatwg.org/#origin-header) | ordinary same-origin GET/HEAD requests are not guaranteed to carry Origin, requiring exact browser-controlled Fetch Metadata as the safe read substitute; checked 10-09-2026 |
| SRC-JOSE-01 | [`go-jose/go-jose` v4.1.4](https://github.com/go-jose/go-jose/releases/tag/v4.1.4) | reviewed RS256/JWK/JWT parser used by the Cloudflare Access fixture adapter; checked 10-09-2026 |
...
| SRC-MAC-04 | [Remote Login](https://support.apple.com/guide/mac-help/allow-a-remote-computer-to-access-your-mac-mchlp1066/mac) | SSH service, declared-user restriction and optional remote Full Disk Access; checked 04-09-2026 |
| SRC-MAC-05 | [Firewall security](https://support.apple.com/guide/security/firewall-security-seca0e83763f/web) and [firewall payload settings](https://support.apple.com/guide/deployment/firewall-payload-settings-dep8d306275f/web) | native application/service firewall behavior; checked 04-09-2026 |
| SRC-MAC-06 | [TN3165: Packet Filter is not API](https://developer.apple.com/documentation/technotes/tn3165-packet-filter-is-not-api) | product-managed PF conflict and support boundary; checked 04-09-2026 |
| SRC-MAC-07 | [Privacy Preferences Policy Control](https://support.apple.com/guide/deployment/privacy-preferences-policy-control-payload-settings-dep38df53c2a/web) | TCC configuration profiles require device management and user approval; checked 04-09-2026 |
...
| SRC-ANS-02 | [Understanding privilege escalation: become](https://docs.ansible.com/projects/ansible-core/devel/playbook_guide/playbooks_privilege_escalation.html) | Ansible executes changing temporary module paths and requires general escalation, so fixed command-path sudo rules do not constrain ordinary become; checked 04-09-2026 |
