...
The following direct user confirmations on 26-08-2026 supersede conflicting site-specific wording in earlier designs. They do not claim implementation, site qualification, phase/batch permission or changes to the historical generated audit register.
...
[Portable lifecycle](platform-lifecycle.md) is the current generic contract. D-118 supersedes mandatory external-workstation bootstrap and universal physical-serial/provider assumptions. D-119 supersedes old connector/marker defaults on 02/08. D-120 supersedes any unconditional claim that restic/R2 no-delete writer compatibility is closed. Original historical decisions/source evidence remain readable; the current requirements and qualified applicability determine new plans.

### Fresh-audit corrections and pending solution choices

On 27-08-2026 the independent requirements audit identified provider-granularity, acknowledgement and stale-sequence conflicts. Corrected requirements require physical 1Password vault partitioning by reader set, truthful tunnel-scoped credential semantics, a separately protected human approval action, preservation of existing control state during Coolify install, and consistent current ledgers. These are audit-derived corrections, not newly claimed user approvals. Exact G-007 layout, G-012 shared-credential/compromise-interruption acceptance and 0.3 account-free human-proof UX remain explicit solution reviews; their affected execution issues cannot be ready until settled. No topology expansion, weaker security or additional cost is authorized. The user was offered the tunnel-boundary choice but no answer was received; do not interpret silence as approval.
...
Generic local setup, separately protected human acknowledgement, native credentials, privilege/OS controls and public build/checks have explicit design/qualification ownership in the [development readiness table](development/roadmap.md#implementation-readiness). They do not acquire a new Labs gate ID merely by being generic requirements. Ask only for ready missing decisions/evidence; never re-ask confirmed outcomes or label missing proof passed.
