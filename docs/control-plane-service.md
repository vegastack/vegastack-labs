...
`POST /api/v1/declarations/{declarationId}/revisions` and `POST /api/v1/declarations/{declarationId}/plans` use the same local server-owned SQLite authority as every other mutation. The declaration ID in the generated route is authorized before strict body decoding and must exactly match the duplicated body binding. The authenticated Console may call these two exact generated operations, but neither handler resolves secret values, invokes an adapter, or performs a network call. The matching exact-revision declaration read and exact-plan read endpoints are available through the generated browser contract.
...
#### Implemented Console change workflow

The `/changes` screen uses only the generated same-origin client to save a declaration, ask the server for current plan-preparation facts, create and review the exact readable/JSON plan, request Slack acknowledgement, watch its safe status, execute the approved plan, and inspect, cancel, or resume the durable run. The browser supplies no human, authority, nonce, proof, policy decision, executor lease, or receipt. `POST /api/v1/plans/{planId}/approval-request` makes the configured server-side acknowledgement adapter prepare and publish the private Slack request; `GET /api/v1/plans/{planId}/approval-status` returns only whether the exact current plan is pending, approved, rejected, or expired and whether apply is currently allowed.

Browser run responses use the separate closed `browser-run` projection. It omits acknowledgement and authorization correlation IDs and the executor binding digest as well as every protected human, authority, nonce, and proof field. The browser route allowlist admits only the named declaration, plan, safe approval, execute, run-read, cancel, and resume operations. Raw acknowledgement submission, executor claims, receipts, direct database access, provider credentials, and policy ownership remain server-only and fail closed at the remote route boundary.

### Implemented durable run engine

The server now owns one in-process durable run engine. `POST /api/v1/plans/{planId}/execute`, `GET /api/v1/runs/{runId}`, `POST /api/v1/runs/{runId}/cancel`, and `POST /api/v1/runs/{runId}/resume` are generated available endpoints. The authenticated Console may call only these exact run operations and receives the safe browser projection described above; external executor mutations remain unavailable on the remote browser listener. Execution loads immutable server state and records a current effective-authorization decision before reading the request body, then binds the exact plan digest, recovery epoch, policy branch, acknowledgement when required, target facts, executor, adapter, operation, artifact, and idempotency key.
...
| `POST /api/v1/plans/{id}/approval-request` | ask the server-owned Slack adapter to publish the exact private acknowledgement request; the browser cannot select the human, authority, nonce, or proof |
| `GET /api/v1/plans/{id}/approval-status` | return the current safe approval state and apply eligibility without protected acknowledgement or policy records |
...
### Implemented Phase 5 operator surfaces

The generated CLI, local API, and embedded Console now expose the same sanitized Phase 5 facts for gates, backups, recovery points, audit checkpoints, restore state, and fixed schedules. Finite collections use the existing signed cursor binding and cap each page at 100 items. Every page remains bound to the caller scope, endpoint and canonical query, grant revision, state revision, recovery epoch, evaluation time, and keyset position; replay after any bound value changes fails instead of mixing snapshots.

Mutation routes name their authorizable resource in the path, or use the fixed server-owned `database`, `audit-checkpoints`, or `profile-drafts` resource. Authentication and route authorization finish before a body byte is read. The handler then validates the closed generated request, rejects a path/body identifier mismatch, and calls the existing server-owned service. The browser write allowlist contains only gate checks, inert gate-evidence drafts, inert restore drafts, and the existing declaration/plan/run controls. Backup, verification, restore execution, database recovery/export, checkpoint creation, and schedule dispatch remain operator-only.

Browser projections omit full policy content, credential and key references, signer/export bindings, fence evidence, private audit payloads, provider errors, filesystem paths, and acknowledgement material. Recovery-required and audit-incident responses preserve the last known safe facts, disable ordinary mutations, and name the exact prerequisite and runbook. They never offer a force path or assert that fixture evidence is live qualification. The Console starts an already prepared exact operation only through the existing Phase 4 plan review and run components; it has no direct executor or SQLite/provider path.

The `database backup` and `database verify` commands are database-scoped aliases over the existing exact approved local-backup effects. Creating a backup leaves its recovery point pending until verification qualifies it. `database restore` creates the existing inert fenced restore plan and does not perform cutover. `database export` records a server-owned `database.export` declaration and returns an explicit draft/change reference; it does not read SQLite into an artifact or claim that an export exists. A later exact plan and separately implemented effect are required before any sanitized export artifact can be produced.
