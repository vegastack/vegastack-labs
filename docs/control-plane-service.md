...
The Unix listener and application start first. Remote asset, identity, certificate, bind, or serving failure changes only `remoteReadState` and `remoteReadReason` in `vsk-labs server status`; it does not stop or widen the local listener. `remoteReadState` is `disabled`, `starting`, `ready`, or `unavailable`, and the reason is a fixed safe value rather than a path, provider response, or raw error. Both listeners drain under the same five-second service shutdown bound.
