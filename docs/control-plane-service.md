...
Browser
  ...
  -> loopback-only vsk-labs server HTTP origin
...
The Console is a control-plane supporting service, not a Coolify workload. A Coolify failure must not remove the diagnostic/control UI. The origin binds only to loopback and the protected Unix socket; `cloudflared` is the only browser ingress. When Cloudflare/Workspace is unavailable, the web UI fails closed and recovery continues through personal SSH plus the Unix-socket CLI or local console.
...
| `assets`, `nodes`, `aliases`, `qualifications` | unique serial and immutable node ID; FQDN, lifecycle, role, capacity, qualification evidence and no alias collision |
...
| `people`, `external_subjects`, `devices`, `grants` | stable provider-neutral subject binding, lifecycle, role/project/shell grants, approver and revision; VegaStack Labs Workspace claims live in the identity-adapter extension; no credential value |
...
| `gate_definitions`, `gate_evidence`, `gate_checks`, `gate_evaluations` | versioned gate/subject/phase effect; collector/attachment digests and restricted references; deterministic observed/expected results, expiry, recovery epoch and current activation state |
...
“Append-only” inside SQLite is an application control, not immutable evidence against a root user or disk compromise. Each audit event therefore includes the previous event hash, sequence and canonical payload hash. The server periodically signs a batch checkpoint with a separately protected signing identity and exports the batch/checkpoint through the outbox to client-side-encrypted, retention-locked off-site storage. Verification detects gaps, reordering, edits and rollback to an older checkpoint; it cannot prove events that an already-compromised server suppressed before export. A recovery compares the local chain with the last independent checkpoint and enters read-only incident mode on any mismatch. No administrator UI may rewrite or “repair” history; corrections are new linked events. [D-106](decisions-and-sources.md#d-106)
...
| Backup repositories | manifest/heartbeat read identity | narrow writer for the declared backup or audit export; no delete/lock-policy grant |
...
4. Copy owned components through the authenticated VegaStack shadcn registry. Registry service-token values remain in local/CI secret storage and are build-time only.
5. Run `vegastack-design doctor` and `check-updates --fail-on-update` in CI. Review diffs before overwrite; copied components are owned source, not silently auto-updated.
...
| Applications | Coolify status, placement, exposure, digest, backup/health and provider deep link; no undeclared placement editing |
...
| People and devices | Workspace identity status, grants, quarantine, onboard/suspend/offboard plans and positive/negative verification |
...
Bootstrap is two-stage because an untrusted candidate cannot nominate itself. A trusted operator workstation performs discovery and the first authenticated installation; after verification, the control service becomes the central executor. [D-101](decisions-and-sources.md#d-101)

```sh
vsk-labs node discover --address <reserved-lan-ip> --output json
vsk-labs node nominate --node vsk-node-04 --role control-plane --output json
vsk-labs control-plane plan --node vsk-node-04 --bootstrap-file <file> --output json
vsk-labs apply --plan-id <plan-id> --output json
vsk-labs control-plane verify --node vsk-node-04 --output json
vsk-labs database backup --verify --output json
```

The bootstrap file is local, schema validated and contains identifiers/references rather than secret values: approved serial/node/FQDN, reserved address and host key, initial admin Workspace identity, protected hostname/Access audience, release identity, 1Password references and backup target. It is retained as encrypted recovery evidence, not as an alternative mutable database.

Safe order:

1. Run `vsk-labs gate check --phase 3 --output json`; verify current passing evidence for the serial map, hardware qualification, LAN reservation, console/SSH trust, supported Debian, time, disk headroom, `G-007`/`G-008` secrets/backup activation and recovery administrator. A documentation-level `design-closed` state is not sufficient.
2. Verify the attested pinned `vegastack-labs` platform release; install the `vsk-labs` executable, unprivileged service account, directories, socket and restrictive firewall.
3. Initialize SQLite, run migrations/integrity checks and insert the instance plus bootstrap administrator as revision 1.
4. Import the approved Sheet-derived inventory with `vsk-labs inventory import --file <approved-export> --output json`; reject duplicates/conflicts, then run `plan --change <draft-id>`. Planning atomically commits the inert desired-state revision and plan; `apply --plan-id` later performs only the approved external transition.
5. Start the API locally; prove socket CLI, authorization and read-only safe mode before enabling browser ingress.
6. Configure 1Password references, encrypted database backup and external watchdog; take and restore the first backup.
7. Install/protect the Tunnel/Access route; prove authorized access, unauthorized denial and direct-origin denial.
8. Reconcile Coolify, Beszel and remote nodes in separate plans. A failed subsystem does not roll back the already-verified database/service foundation.
9. Run end-to-end create-plan/acknowledge/execute/verify tests on a disposable target and record audit/export evidence.

Physical cabling, the serial conflict, initial Debian/console trust, Workspace administration, 1Password recovery custody and destructive acknowledgement remain human gates. The CLI reports them as blockers; it does not invent or bypass them.
...
5. Administratively fence the former instance outside the copied database: stop/disable or quarantine its host and Mesh/network identity; revoke its 1Password resolver/service identity; revoke or rotate every mutating provider/API, Coolify, SSH, backup and audit-export credential it could resolve; remove its control SSH key from managed nodes. If the host is lost, provider administrators and trusted-workstation/console runbooks perform those revocations before proceeding.
...
7. Issue only new-instance credentials with the documented narrow scopes, re-resolve their references from 1Password and independently prove old credentials/host paths are denied at every mutating adapter and managed-node boundary. Epoch comparison protects the recovered database; credential/network revocation protects against a still-running old copy.
...
If no database backup survives, initialize a clean database from the latest signed declarative snapshot, rebind the bootstrap admin, refresh providers and reconstruct runtime state. This loses unexported operational history and approvals; all old plans are invalid and no mutation is permitted until the recovery audit is accepted.
...
- component CI passes typecheck, build, VegaStack doctor/drift, unit/integration, keyboard, axe, light/dark and mobile/desktop visual gates.
