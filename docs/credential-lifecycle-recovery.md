...
2. Before planning, have the independent administrator establish the stable source admission: former and replacement identities, exact draft fingerprint and epoch transition, witness and recipient public keys, administrator root, complete denial requirements, and signed-adapter qualification digest. The request fields named `custodyProofDigest` and `formerControllerFenceDigest` carry this source-admission digest and qualification digest for wire compatibility; they are not post-plan witness or envelope digests.
3. Import a new inert draft under the replacement host key in the already-current epoch. Submit `vsk-labs credential recover` with the exact prior epoch and stable admission pair. Inspect the plan's draft origin, ciphertext fingerprint, target, revision, both epochs and admission pair. After the immutable plan exists, have the independent administrator install the signed public manifest, signed witness and encrypted custody envelope at the fixed protected recovery paths. These artifacts must bind the exact resulting plan, run step, lease and the same admission pair. The replacement recipient key comes from its protected service credential. Do not copy the former host key or place opened material in a file.
4. After independent human acknowledgement, apply once. The installed recovery source must rederive and match the stable admission pair, independently validate the actual signed manifest, witness and envelope, record their actual artifact digests, verify freshness and the complete denial set, consume its one-use receipt, open the exact protected ciphertext inode, and compare the stream with independently supplied material. A successful effect appends a `staged` version and recovery evidence only. Activate it through a separate plan after native positive and denied verification.
