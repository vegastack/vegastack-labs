...
Issue #30 exclusively owns SQLite initialization, migration and transaction primitives. Issue #34 consumes its supported Online Backup API seam rather than copying a live database. Issue #31 owns generic inventory-draft schemas and validation; #32 maps sanitized CSV into those contracts without adding Labs fields to core types. Issue #33 owns event/outbox atomicity. Issue #35 composes authorized reads into the existing `Application` route, and #36 consumes those reads through the existing local client. Issue #37 owns canonical declaration export/signing but no private key or database dump.
