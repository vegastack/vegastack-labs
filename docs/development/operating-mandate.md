...
During the Phase 5 delivery batch, run one successful exact-base/exact-head affected public check immediately before asking the operator to create an intermediate issue's pull request. Changed Go package tests and vet, repository-wide compilation, and the affected tooling, web, browser, phase and boundary checks form that proof. PR and post-merge CI use the same affected planner for their own exact base/head and install and run browser tests only for browser-impacting changes. Run the complete public suite once for the final Phase 5 integration or acceptance candidate, selected explicitly; never create an implicit full run by omitting the base. Unknown paths, package deletion or rename, dependency/selector changes, invalid ancestry, or malformed proof fail closed to a full plan or block shipping. If review, rebase, or conflict-resolution work changes the head, rerun the affected proof. Pull requests run on fresh GitHub-hosted Ubuntu 24.04 machines. Under the temporary Issue #81 exception, trusted `main` pushes and explicit manual runs may use disposable `vsk-node-01` or `vsk-node-06`; the job verifies the exact hostname before checkout and receives no fleet-admin or deployment credentials. An issue's named native or real-boundary acceptance command remains separately required.
