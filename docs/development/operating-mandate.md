...
Run one successful complete `pnpm check` at the exact clean branch head immediately before asking the operator to create a pull request. Do not repeat the complete lane as a ritual during implementation. If review, rebase, or conflict-resolution work changes that head, rerun only the affected checks; PR CI verifies the actual candidate through `pnpm check:affected`. The GitHub-hosted macOS 15 primary job runs the selected public checks. A small GitHub-hosted Ubuntu 24.04 Linux compatibility job runs `go test ./...` only for Go/Linux-impacting changes selected by the same immutable plan and never uses fleet machines. PR and post-merge CI install and run browser tests only for browser-impacting changes. Missing, invalid, or unclassifiable diff evidence fails closed to the full suite, including Linux coverage. An issue's named phase-acceptance command remains separately required.
