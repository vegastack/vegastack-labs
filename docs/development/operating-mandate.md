...
Run one successful complete public check at the exact clean branch head immediately before asking the operator to create a pull request. The repository profile selects its proof route: either the pinned local `pnpm check` or a successful manual Public CI run whose exact-head structured result proves the same `runCheckPlan(fullCheckPlan())` lane. Do not repeat the complete lane for an unchanged head. If review, rebase, or conflict-resolution work changes that head, rerun only the affected checks; PR CI verifies the actual candidate through `pnpm check:affected`. Pull requests run on fresh GitHub-hosted Ubuntu 24.04 machines. Under the temporary Issue #81 exception, trusted `main` pushes and explicit manual runs may use disposable `vsk-node-01` or `vsk-node-06`; the job verifies the exact hostname before checkout and receives no fleet-admin or deployment credentials. PR and post-merge CI install and run browser tests only for browser-impacting changes. Missing, invalid, or unclassifiable diff evidence fails closed to the full suite. An issue's named phase-acceptance command remains separately required.
