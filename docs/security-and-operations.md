...
The native consumer and clean-host recovery verifiers are implemented, but the live-evidence gate and production recovery-source registry remain unavailable until their exact site adapters and independent authority are enrolled and qualified. Activation, rotation and recovery therefore fail closed when required live proof is unavailable; isolated fixture evidence grants no live authority. For a denied, failed or interrupted operation, preserve the import, declaration, binding, plan and sanitized run evidence. Reinspect current state and follow the [credential lifecycle and recovery procedure](credential-lifecycle-recovery.md); never overwrite ciphertext, edit SQLite, widen a consumer set, silently revoke the prior version, or treat a draft as applied.
