...
Issue #106 adds the Phase 5 local recovery-point **creation** path and nothing beyond it. An operator stages an inert canonical backup policy with `vsk-labs backup policy draft --config <profile> --file <exact-json>`; the server strictly validates the policy (owner, registered source and repository IDs, bounded selectors, registered consistency hook, class `none|standard|critical`, key references, retention and dependencies), stores canonical bytes, and returns an opaque draft ID and SHA-256 digest. That digest binds through exactly one `x-backup-policy` extension on the existing generic declaration; planning and execution re-read the exact stored bytes and deny a missing, changed, cross-revision or cross-epoch binding. A policy names registered IDs only and can never supply a host repository root or a restic executable path — the protected server profile owns the absolute standard/critical backup roots and the one pinned restic `0.19.1` binary path (verified by owner, mode, filesystem and architecture-pinned executable digest before each effect). Execution runs solely through the exact Phase 4 plan/run/human-acknowledgement path: it captures the declared source through the store-owned SQLite Online Backup consistency hook, runs the pinned restic child against a same-process, lease-bound Unix REST object boundary (the routine writer creates new payload and refreshes/removes only mutable `locks/`; it can never overwrite or delete retained `config`, `keys/`, `data/`, `index/` or `snapshots/`), delivers the repository password only through a sealed anonymous memory FD (`--password-file /proc/self/fd/N`; never an argument, environment variable, file, SQLite column, log, audit record or output), binds canonical creation manifest v1.1 with the exact source, snapshot, immutable object and complete policy-declared dependency inventories, and durably records one **pending** recovery point. The central typed execution receipt carries only the opaque pending point ID and manifest digest. A declared signature dependency is historical provenance, not current trust: #117 must independently prove its current trust root/signature binding and fail closed if unavailable. A pending point is never local last-good, recovery-qualified, off-site or live `G-008` evidence, and an exit-0 restic command or green metadata check does not qualify it; #117 owns the independent full-read/isolated-restore verification and last-good advancement, #114 owns off-site generation, and #115 owns retirement. On any failure — full destination, wrong key/binary/format, corrupt or missing object, live-lock conflict, ambiguous publish or hook failure — creation blocks without pruning and every prior point is preserved. The Linux restic-against-temporary-repository lane is acceptance-only and never uses inventory hosts or live storage.
...
Issue #117 adds local point verification through the same exact human-approved plan executor. A `backup.local.verify` operation names one pending point, its manifest digest and expected inventory digest; the server checks its current policy, state revision, recovery epoch and opaque key reference before acquiring a read lease that excludes writers. The same-process REST role permits retained reads and only its own lock lifecycle. The verifier hashes every object in that point's canonical inventory, requires the exact restic snapshot, runs pinned `check --read-data`, restores that snapshot into an owner-only temporary sibling and checks the recovered SQLite content, integrity, foreign keys, schema catalog and revision without writing the authoritative database. Objects added by later valid points may coexist; a missing or changed object in this point's manifest denies verification. Every attempt is immutable and digest-bound. A passed isolated fixture remains `fixture-only`; only a current protected-server proof can advance class-scoped local last-good by compare-and-swap. The policy's v1.2 full-payload and functional-test intervals produce due times from recorded checks; an overdue, failed, uncertain, stale or fixture proof cannot become a current live claim, and prior last-good remains recorded after failure.
