...
The finite custodian-side recovery witness collector accepts a protected administrator-signed pin from fixed OS paths, a public exact-attempt and boundary document, and separate inherited descriptors for the witness seed and recovery material. It bounds both private reads, closes them on cancellation and failure, rejects a substituted seed, requires every typed direct-denial response and a second direct probe, and emits only a signed public artifact plus encrypted replacement-recipient envelope. Its production adapter registry is empty until independently administered custody and each old-identity denial mechanism are qualified; synthetic Linux endpoints demonstrate protocol behavior but grant no recovery or `G-007` authority. The existing recovery-required/read-only state remains the safe result of any missing custodian, pin or probe.
