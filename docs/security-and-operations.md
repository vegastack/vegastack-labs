...
Before enabling the adapter, the operator sets the optional absolute `slackAcknowledgementConfigPath` in the server profile. That strict, bounded JSON file contains only the exact Slack identifiers and three distinct logical references: app token, bot token, and nonce key. On supported Debian the corresponding material is delivered to the service by systemd's native credentials directory; files must be non-linked regular files owned by the service UID with no group/other permissions. The nonce key derives a different in-memory nonce from the exact request, so restart/retry remains deterministic while only its digest reaches SQLite. Missing/invalid configuration or native credential material leaves the local core running but the acknowledgement request blocked.
