...
Use the protected operator client with a current revision and recovery epoch, an exact stored import draft for stage/rotate/recover or exact applied version for activate/revoke, and the scoped lifecycle-author grant. The request is the strict [lifecycle metadata contract](../schemas/v1/credential-lifecycle-request.schema.json), schema version 1.2.0; its target digest seals public metadata and its consumer sets are bounded and disjoint. No request accepts a value, ciphertext fingerprint, import-origin fields or acknowledgement. The browser cannot submit this endpoint.
