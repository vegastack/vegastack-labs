...
This foundation does not accept plaintext input, generate a value, activate a reference, rotate or revoke material, write credential status, register a production resolver, or admit a secret-bearing production effect. Those lifecycle operations require their later approved issues plus current `G-007` evidence. No live token, host key, provider account, vault or credential was used to verify this foundation.
