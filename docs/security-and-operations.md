...
Audit is **tamper-evident**, not absolutely immutable. SQLite service permissions prevent routine update/delete, while every canonical event hash-chains to its predecessor. A separately protected signing identity signs periodic sequence/checkpoint digests; a dedicated export writer may add encrypted audit batches to the six-month retention-locked R2 namespace but cannot read backup content, delete objects or change the lock. Verification compares local sequence/hash/signature with the independent last checkpoint and enters read-only incident mode on gaps, edits, reordering or rollback. A root compromise can suppress an event before export, so short export intervals, off-site silence monitoring and preserved provider-native event IDs remain necessary. Corrections are new linked events, never history rewrites. [D-106](decisions-and-sources.md#d-106)
