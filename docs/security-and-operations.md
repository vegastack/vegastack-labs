...
Before enabling the adapter, the operator sets the provider-neutral optional absolute `acknowledgementAdapterConfigPath` in the server profile. The selected deployment-profile adapter owns that file's strict schema; the v1 Slack file contains only exact Slack identifiers and three distinct logical references: app token, bot token, and nonce key. On supported Debian both the mapping file and corresponding systemd native credential files must be non-linked regular files owned by the service UID with only the documented `0600`/`0640` mapping permissions and no group/other credential permissions. The nonce key derives a different in-memory nonce from the exact request, so restart/retry remains deterministic while only its digest reaches SQLite. Missing/invalid configuration or native credential material leaves the local core running but the acknowledgement request blocked.

Before rollout, verify an independent local recovery path and exercise approve, reject, outage, reconnect, restart, expiry, and replay denial against a non-production test plan. If the connection becomes unavailable, repair the declared provider path and restart the same server process; do not reveal credentials, reuse a rotating URL, edit acknowledgement rows, or bypass the proof. Database rollback follows the standard verified pre-migration-copy procedure and must preserve the durable audit/history evidence.
...
The dormant Go resolver uses the pinned official `github.com/1password/onepassword-sdk-go` `v0.4.1` module and permits one exact `op://vault-id/item-id/field-id` read only after applied profile, capability, consumer, plan, lease and recovery-epoch checks. It never enumerates vaults or falls back to a label lookup. Physical-vault reader sets remain the real provider permission boundary: an item or field ID does not narrow a stolen service-account token. Provider rate limiting, dependency outage and authorization denial return distinct stable redacted codes; loss of the optional adapter blocks only operations that depend on it. App-owned byte buffers are wiped on normal, error and panic exits. The SDK returns immutable Go strings, so the process cannot promise an immediate overwrite of SDK-owned memory; it avoids retaining or logging those strings and relies on Go reclamation after copying into the bounded application-owned value. [1Password Go SDK v0.4.1](https://github.com/1Password/onepassword-sdk-go/tree/v0.4.1)

On supported Debian, staging invokes `systemd-creds encrypt --with-key=host` through a direct argument array and passes material on bounded standard input, never in an argument. The ciphertext directory and files must be non-linked regular objects with the expected owner, mode, bound name, version and fingerprint; resolution repeats those checks. `--with-key=null` is forbidden. Host-key encryption does not protect against root or whole-disk theft when the key is on the same disk, and TPM hardware remains optional rather than silently required. A replacement host needs independently authorized recovery and re-encryption under its newly qualified host key; copying the old host key is not recovery proof. [systemd credentials](https://systemd.io/CREDENTIALS/)

The local `credential import` command accepts bounded private bytes only from standard input or an explicitly opened descriptor. It never accepts a private argument, environment value, JSON field, named-file/path flag, browser request or constrained-SSH request. The server checks local identity, public metadata, current revision/epoch, idempotency and the existing ciphertext state before reading the body; exact retries return the sanitized recorded draft without reading a replacement value. Application-owned buffers are wiped on normal, error and panic exits, while stdout, stderr, audit and database records contain only public bindings and nonreversible ciphertext fingerprints.

Import is not activation. It writes one append-only inert draft row and one host-key ciphertext, but it does not generate a value, activate a reference, rotate or revoke material, write live credential status, register a production resolver or admit a secret-bearing production effect. Any missing, corrupt, linked, mis-owned or mode-invalid ciphertext; orphaned ciphertext without its database row; binding/fingerprint mismatch; stale recovery epoch; or post-promotion database failure returns a stable denial, including `RECOVERY_REQUIRED` for indeterminate/orphaned state. Preserve both sides and their sanitized evidence. Issue #124 deliberately has no delete, overwrite or reconciliation path; a human follows the separately approved #125 lifecycle recovery rather than repairing files or SQLite directly.

The systemd host-key threat boundary is unchanged: same-disk root or whole-disk compromise can obtain both ciphertext and key, and a fixture/import success says nothing about independent recovery custody. A replacement host must use separately retained recovery material and authorized re-encryption under its own newly qualified host key. These lifecycle operations still require their later approved issues plus current `G-007` evidence. No live token, host key, provider account, vault or credential was used to verify this import boundary.

The finite custodian-side recovery witness collector accepts a protected administrator-signed pin and complete requirement set from fixed OS paths, a public exact-attempt binding, and separate inherited descriptors for the witness seed and recovery material. A caller cannot remove a boundary from the signed set. It bounds both private reads, closes them on cancellation and failure, rejects a substituted seed, requires every typed direct-denial response and a second direct probe, and emits only a signed public artifact plus encrypted replacement-recipient envelope. Its ordinary production build has an empty adapter registry; a disposable-only build tag provides a loopback synthetic adapter for a root-container multi-process test under distinct custodian, former and replacement identities. This demonstrates software behavior but grants no recovery or `G-007` authority. The existing recovery-required/read-only state remains the safe result of any missing custodian, pin or probe.
...
Issue #106 adds the Phase 5 local recovery-point **creation** path and nothing beyond it. An operator stages an inert canonical backup policy with `vsk-labs backup policy draft --config <profile> --file <exact-json>`; the server strictly validates the policy (owner, registered source and repository IDs, bounded selectors, registered consistency hook, class `none|standard|critical`, key references, retention and dependencies), stores canonical bytes, and returns an opaque draft ID and SHA-256 digest. That digest binds through exactly one `x-backup-policy` extension on the existing generic declaration; planning and execution re-read the exact stored bytes and deny a missing, changed, cross-revision or cross-epoch binding. A policy names registered IDs only and can never supply a host repository root or a restic executable path. A separate root-owned custody profile fixes those paths, the executable inode/digest, controller/custody/restic UIDs, exact systemd template and maximum lifetime. Execution captures the declared source through the store-owned SQLite Online Backup consistency hook, journals the exact attempt, and asks systemd to start only `vsk-labs-backup-custody@<32-hex>.service`. Before that start, the controller proves its sole sudo allowance is the same binary's argument-free `__backup-custody-policy-check` mode; that root read-only helper binds the caller's live PID/start time/UID and proves the exact polkit start grant while adjacent verbs, units and broader systemd actions deny. The same signed executable's finite supervisor then authenticates the controller peer and nonce, launches one locked custody-UID child, and runs the verified restic inode under the distinct restic UID. The repository password crosses only as a sealed anonymous memory FD (`--password-file /proc/self/fd/N`; never an argument, environment variable, file, SQLite column, log, audit record or output). The controller and restic cannot traverse the custody-owned repository or quarantine; inventory, capacity and results return through bounded authenticated IPC. The writer creates new payload and refreshes/removes only its mutable locks; it cannot overwrite or delete retained `config`, `keys/`, `data/`, `index/` or `snapshots/`. The server binds canonical creation manifest v1.1 and durably records one **pending** recovery point. A pending point is never local last-good, recovery-qualified, off-site or live `G-008` evidence. On full destination, wrong key/binary/format, corrupt object, lease loss, child crash or ambiguous response, the attempt fails or remains uncertain without pruning and every prior point is preserved. The distinct-UID systemd/restic proof is acceptance-only and never authorizes accounts, units, storage or inventory-host changes.
...
Issue #117 adds local point verification through the same exact human-approved plan executor. A `backup.local.verify` operation names one pending point, its manifest digest and expected inventory digest; the server checks its current policy, state revision, recovery epoch and opaque key reference before acquiring a read lease that excludes writers. The verifier starts the same finite custody boundary in read role; retained reads and the verifier's own lock lifecycle occur under the custody UID, while pinned restic remains a separate socket-only peer. The controller hashes no repository path directly. It receives exact typed inventory, requires the restic snapshot, runs `check --read-data`, restores into one fresh fixed exchange child, and checks the returned controller-owned SQLite content, integrity, foreign keys, schema catalog and revision without writing the authoritative database. The root supervisor opens that exact child beneath the fixed exchange root with `openat2` beneath/no-symlink constraints, validates it with `fstat`, and retains the directory and snapshot descriptors through restic execution. Ownership transfer and return use only `fchown` on those retained handles; a rejected request has no cleanup or ownership path. Objects added by later valid points may coexist; a missing or changed object in this point's manifest denies verification. Every attempt is immutable and digest-bound. A passed isolated fixture remains `fixture-only`; only a current protected-server proof can advance class-scoped local last-good by compare-and-swap. The policy's v1.2 intervals produce due times from recorded checks; an overdue, failed, uncertain, stale or fixture proof cannot become a current live claim, and prior last-good remains recorded after failure.

Local retirement is human-only and exact. The server selects points relative to the latest verified point, preserves the sole last-good and every point in the current applied complete retention-lock catalog, forecasts retained growth and one bounded repack with 30% headroom, and stores an inert intent bound to the full point set, object inventory, catalog digest/sequence, source-coverage digest, revision and epoch. Execution requires a current destructive infrastructure-admin acknowledgement, central run and exact lease. It starts custody with the distinct `retention` role; the routine writer and verifier cannot invoke `forget` or `prune`. Before any effect it compares the complete strict snapshot inventory. Every dynamic restic PUT and DELETE is write-ahead journaled with exact name, digest and bytes. DELETE renames the verified object into a custody-owned same-filesystem quarantine; it does not physically unlink it. A survivor dependency, stale catalog, added snapshot, missing proof, bound overflow, denied mutation or lost response stops settlement.

Retirement succeeds only after the exact targets disappear, every survivor remains, the complete post-object inventory is recorded, pinned restic completes `check --read-data`, and every surviving last-good point passes isolated restore. Custody must close successfully before one atomic successor generation and verified receipt release the exclusive lease. Quarantined bytes remain recoverable and count toward capacity; physical unlink is a separate future bounded finalization. The sanitized `backup status` projection reports intent, catalog, target/survivor IDs, forecast and receipt digests without a direct DELETE or status-edit route. Manual fallback uses the same generated status, current catalog, fresh exact plan, human acknowledgement, retention credential and custody flow. After an uncertain response, preserve quarantine and the prior last-good, inspect the journal and full inventory, and use a fresh human plan; never retry restic directly or infer success from exit 0 or prose.

If custody startup or IPC fails, leave the journaled attempt failed or uncertain and preserve every retained object and prior last-good point. An infrastructure administrator checks the fixed profile, executable digest/inode, account IDs, directory ownership/modes, exact sudoers line, systemd template and polkit allow/deny matrix, then retries through a fresh approved plan and nonce. Direct restic, direct repository permission changes, a broader sudo/polkit grant, manual lock deletion and database edits are not recovery shortcuts.

The protected verifier has independent local trust for the pinned restic binary and current SQLite schema catalog. For a policy dependency of kind `config`, `image` or `signature`, the current-trust resolver requires one revisioned source selected for the exact dependency. Its public draft names the artifact digest, Sigstore bundle digest, trust-root reference and digest, signer identity and issuer; applying or revoking that draft is an append-only, exact plan/run/step/lease transition at the current state revision and recovery epoch. Verification reads artifact, bundle and root bytes only through the compatible registered reader, rehashes all three, verifies the bundle with the registered identity/issuer/root, and appends secret-free point/policy/revision/epoch-bound evidence. Missing, stale, changed, revoked, wrong-root or forged input records only the sanitized reason `backup-dependency-trust`; it cannot advance last-good or erase the previous good point.

No production artifact reader or site source/root registration path is installed yet. To establish one, the backup administrator must first obtain separate approval for a typed protected reader and the site trust root, then submit the exact public source metadata as an inert revisioned draft and make it current through the normal declaration, immutable plan, independent human acknowledgement and run path. Re-read the current source by dependency and revision before verification. Never register by editing SQLite, pass artifact/root bytes in a verification request, accept a manifest's self-assertion, or promote a fixture. If registration or verification fails, preserve the current source record and prior last-good, inspect `vsk-labs backup status --config <profile> --output json` for the sanitized attempt, correct or revoke the source through a new approved plan, and retry the point only with a fresh plan and acknowledgement. Until that separately reviewed registration surface and reader exist, stop and leave the dependency unqualified.

The synthetic Linux signed fixture and restored SQLite prove software behavior only. They do not provide a site root, real media, off-site, independent key, clean-host or live `G-008` evidence. For an otherwise eligible failed local verification, the human operator selects the exact pending job/point and manifest digest, checks the policy/key reference and current epoch, obtains a fresh plan and human acknowledgement, then uses `vsk-labs backup verify --config <profile> --file <exact-json> --output json`. A failed or uncertain result stops there. Manual fallback follows those same prerequisites and checks through the approved `vsk-labs` flow; direct restic, SQLite edits, prune and restore cutover are separate break-glass/reconciliation procedures, not a shortcut for this verification.

The selected candidate is restic repository format v2; the writer/lock/retained-payload compatibility mechanism remains open under G-008 and must be proven without weakening safety. Use separate `standard`, `critical-local` and `critical-offsite` repositories. Pin restic `0.19.1` or a reviewed later patch by binary digest; pass the narrow referenced password through an inherited sealed anonymous FD, never `RESTIC_PASSWORD_COMMAND`; use `keep-within` retention; and separate routine writer from retention/prune and R2 lock administrators. Before phase 3, `G-008` still requires actual repository IDs, measured capacity, R2 credential/lock evidence and functional sealed recovery without the control node. [Implementation gates](implementation-gates.md#backup-engine-and-repository-topology--g-008) [D-109](decisions-and-sources.md#d-109)
...
The implemented audit layer atomically stores one bounded canonical event, one instance/epoch-bound hash-chain link and the required durable outbox rows with each business intent. SQLite serializes predecessor selection, links and events are append-only, and migration backfill preserves old events as clearly labelled `preAnchor` history rather than claiming that they were independently proven. Corrections remain new same-target events. Public reads expose only IDs, hashes, status and fixed reason codes—never raw errors, payload maps, provider responses, paths, prompts or secrets.

Checkpoint creation is an inert declaration followed by the normal exact-plan path. The core has separate signer, append-only encrypted writer and independent read-only interfaces; a checkpoint remains `pending`, `signed` or `export-pending` until a separately read copy matches the exact instance, epoch, range, key version, signature and namespace. An unavailable signer/export/read adapter is degraded or pending and does not disable unrelated local operation. The shipped production composition has no live audit signer, exporter, namespace or public key, so synthetic tests do not claim an independent anchor, retention lock, silence monitoring or G-007/G-008 closure. A root or disk compromise before an independent anchor remains outside application controls. [D-106](decisions-and-sources.md#d-106)

`vsk-labs audit checkpoints` and `vsk-labs audit verify` use the same generated API as the Console-facing read boundary. Verification checks every local payload/context/link, anchored range and independently signed checkpoint. A missing independent read reports `degraded`; a proven edit, gap, fork, bad signature or ahead/conflicting checkpoint enters read-only `audit-incident` mode without selecting or rewriting either history. On incident, preserve the authority file and independent object, fence the former controller and its mutation credentials, compare the signed checkpoint outside the affected authority, verify a matched database and recovery decision, then create a new recovery-epoch segment bound to the prior checkpoint. Never truncate, repair in place, copy a former instance identity or replay provider mutations merely to fill an audit suffix.
...
### Human procedure for credential lifecycle drafts

All five lifecycle actions are control-plane risk. A plan is eligible for that classification only when it carries one sealed `core.credential` lifecycle operation; it remains centrally executed on the human authorization branch. The assigned human control-plane-admin acknowledges the exact plan target with an active `acknowledge` grant for `plan.acknowledge`/`plan-target`, and the executor needs a separate active `execute` grant for the exact `credential.<action>`/`execution-target` and target. A general `plan.execute` grant, author role or infrastructure-admin role does not authorize lifecycle apply. An agent cannot acknowledge; it can execute only with its own exact action-scoped control-plane-admin grant after the independent human acknowledgement is consumed. Lifecycle actions are never policy-preauthorized. No live grant is created by submitting a draft.

Credential plans generated before this classification may carry `routine` or `destructive` risk and fail closed; regenerate those plans from current state. Preserve existing applied rotation history and all independent recovery prerequisites.

Use the protected operator client with a current revision and recovery epoch, an exact stored import draft for stage/rotate/recover or exact applied version for activate/revoke, and the scoped lifecycle-author grant. The request is the strict [lifecycle metadata contract](../schemas/v1/credential-lifecycle-request.schema.json), schema version 1.3.0; its target digest seals public metadata and its consumer sets are bounded and disjoint. Native activate/rotate requests must seal each local systemd unit, service UID/GID and required denied reader identity. The server also seals the one ciphertext artifact consumer from the staged reference for activate or immutable import draft for rotate; every mapped unit uses the same name-bound loaded credential. The plan read and append transaction recheck that origin. This mapping is a plan claim, not runtime proof: native activation stays unavailable until #141 verifies real systemd delivery and denied reads. No request accepts a value, ciphertext fingerprint, import-origin fields or acknowledgement. The browser cannot submit this endpoint.

1. Submit one selected action through vsk-labs credential stage, activate, rotate, revoke or recover, with --config pointing to the protected profile and --file pointing to the metadata request. Use the identical action in the file. Save the returned change ID, operation ID, revision and epoch; draft status changes no credential status. An exact retry returns the same inert submission without another declaration or binding.
2. Create the ordinary plan through vsk-labs plan with the same --config, --declaration-id selecting the returned change ID, and --revision 1. Inspect its exact target, action, material version, consumer verification requirements, overlap and recovery prerequisites through the shared readable/JSON plan. Another desired-state change can make the draft stale; regenerate from current state instead of changing its sealed revision.
3. Obtain normal independent human acknowledgement through the configured acknowledgement adapter, then execute only that immutable plan through vsk-labs apply with the same --config and --plan-id selecting that plan. The client, Console and agents cannot self-acknowledge.
4. Inspect the durable run through vsk-labs run inspect with the same --config and --run-id selecting the returned run, and verify the exact version postcondition. Staging a replacement leaves the current active version selected. Applied rotation changes logical current while preserving the prior exact status until independently planned named revocation; revoking the prior cannot select it again.

The shipped server still has no qualified production consumer, gate or clean-host recovery verifier. Activation, rotation and recovery fail closed when their required proof is unavailable; isolated fixture evidence grants no live authority. For a denied, failed or interrupted operation, preserve the import, declaration, binding, plan and sanitized run evidence. Reinspect current state and use the ordinary plan/run recovery rules; never overwrite ciphertext, edit SQLite, widen a consumer set, silently revoke the prior version, or treat a draft as applied. Independent clean-host custody and former-controller fencing remain prerequisites owned by the subsequent recovery issue.

### Independent recovery source handback

The protected-source reader is dormant software for a clean replacement. It cannot authorize recovery by itself. Before a production attempt, an infrastructure administrator outside both controllers must provision the existing admin signing root, witness signing custody and replacement recipient key, then sign one manifest for the exact draft, ciphertext fingerprint, plan, run, step, lease, revision, epochs and complete current boundary set. The custodian collects fresh typed direct-denial transcripts and an encrypted envelope; an independent administrator installs only the signed public artifacts and ciphertext under the fixed, root-owned `/etc/vsk-labs/recovery/` directory. No private key or opened material enters a file there, restored SQLite, a request, a log or a command argument.

For each configured boundary, the administrator must separately qualify a reviewed, digest-pinned typed verifier for the exact target and former identity. The replacement derives required boundaries from current applied state, reads the protected package from pinned inodes, verifies the admin manifest and qualification record, re-probes every endpoint under the former identity, and checks the exact signed witness and recipient-bound envelope. A missing real verifier, stale observation, surviving former session, Mesh re-enrollment or allowed writer leaves the source unavailable. Ordinary production currently registers no verifier, so this procedure cannot yet be used to recover.

Only after #144 binds the opened ciphertext to the exact protected inode and #108 proves current replacement authority may the replacement consume the one-use OS receipt and pass opened material to the exact comparator. A failed comparison or interrupted receipt claim is uncertain and must not be retried with the same challenge or receipt. Preserve the signed public artifacts and sanitized digests, inspect current state, and start a newly authorized attempt with new evidence; never promote the fixture, restore an old controller's authority, or infer `G-007`/`G-008` from a software test.
