...
The protected verifier currently has independent local trust for the pinned restic binary and current SQLite schema catalog. A policy dependency of kind `config`, `image` or `signature` has no current trusted resolver and denies live qualification; its manifest digest remains historical provenance. The synthetic Linux acceptance repository and restored SQLite prove software behavior only. They do not provide real media, off-site, independent key, clean-host or live `G-008` evidence. To retry a failed local verification, the human operator first reads `vsk-labs backup status --config <profile> --output json`, selects the exact pending job/point and manifest digest, checks the policy/key reference and current epoch, obtains a fresh plan and human acknowledgement, then uses `vsk-labs backup verify --config <profile> --file <exact-json> --output json`. A failed or uncertain result stops there: preserve the prior last-good, inspect the sanitized attempt, correct the cause, and create a new current plan. Manual fallback follows those same prerequisites and checks through the approved `vsk-labs` flow; direct restic, SQLite edits, prune and restore cutover are separate break-glass/reconciliation procedures, not a shortcut for this verification.
