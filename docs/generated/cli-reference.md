...
### `vsk-labs backup offsite-retirement dry-run`

Derive one exact off-site retirement intent from the qualified complete bucket catalog without staging it.

Owner phase: `5` · risk: `read-only` · availability: `available`

- `--config <path>` — Read one protected local server profile.
- `--file <path>` — Read one exact typed backup-offsite-retirement-dry-run-request JSON file (64 KiB max).
- `--output <format>` — Select human or versioned JSON output. Allowed: `human`, `json`.
- `--schema-version <major>` — Select the machine-contract schema major. Allowed: `1`.

- Derive one exact off-site retirement intent from the qualified complete bucket catalog without staging it.: `vsk-labs backup offsite-retirement dry-run --config fixture/server-profile.json --file fixture/backup-offsite-retirement-dry-run-request.json --output json`
...
Create one immutable fenced restore plan.

Owner phase: `5` · risk: `mutation` · availability: `available`

- `--config <path>` — Read one protected server profile.
- `--file <path>` — Read one exact material-free restore request JSON file (256 KiB max).
- `--output <format>` — Select human or versioned JSON output. Allowed: `human`, `json`.
- `--schema-version <major>` — Select the machine-contract schema major. Allowed: `1`.

- Create one immutable fenced restore plan.: `vsk-labs restore plan --config fixture/server-profile.json --file fixture/restore-plan-request.json --output json`

### `vsk-labs restore run`

Stage one exact authorized restore candidate.
...
- Stage one exact authorized restore candidate.: `vsk-labs restore run --config fixture/server-profile.json --file fixture/restore-run-request.json --output json`

### `vsk-labs restore verify`

Verify the recovered authority and complete its canary.
...
- Verify the recovered authority and complete its canary.: `vsk-labs restore verify --config fixture/server-profile.json --file fixture/restore-verify-request.json --output json`
...
### `vsk-labs schedule inspect`

Inspect one sanitized fixed scheduled policy.

Owner phase: `5` · risk: `read-only` · availability: `available`

- `--config <path>` — Read one protected local server profile.
- `--output <format>` — Select human or versioned JSON output. Allowed: `human`, `json`.
- `--policy-id <id>` — Select one exact scheduled policy.
- `--schema-version <major>` — Select the machine-contract schema major. Allowed: `1`.

- Inspect one sanitized fixed scheduled policy.: `vsk-labs schedule inspect --config fixture/server-profile.json --policy-id policy-a --output json`

### `vsk-labs schedule list`

List sanitized fixed scheduled policies.
...
- List sanitized fixed scheduled policies.: `vsk-labs schedule list --config fixture/server-profile.json --output json`
