...
func TestPhase5GeneratedNamesMatchEveryConsumer(t *testing.T) {
	...
	if phase5 != 23 {
		t.Errorf("Phase 5 endpoint count = %d, want 23", phase5)
	}
...
func TestGeneratedContractsPreservePublicBoundary(t *testing.T) {
	...
	if available != 34 || planned != 31 {
		t.Fatalf("command availability = (%d available, %d planned), want (34, 31)", available, planned)
	}
	...
	if !reflect.DeepEqual(availablePhase5, []string{"audit checkpoints", "audit verify", "backup policy draft", "backup run", "backup status", "backup verify", "credential activate", "credential import", "credential recover", "credential revoke", "credential rotate", "credential stage", "gate check", "gate evidence", "gate inspect", "gate list", "gate profile draft"}) {
