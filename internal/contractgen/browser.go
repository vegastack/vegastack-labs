...
func renderBrowserContractGraph(endpoints []metadata.EndpointDefinition, schemas []metadata.SchemaDefinition, errors []metadata.ErrorDefinition, lifecycle metadata.LifecycleDefinition) []byte {
	...
	output.WriteString("// Browser-safe available reads and planned Phase 4/5 contracts:\n")
	for _, endpoint := range endpoints {
	...
	for _, group := range []struct {
		...
		{"GATE_EVIDENCE_TRANSITIONS", lifecycle.GateEvidenceTransitions},
		{"BACKUP_JOB_TRANSITIONS", lifecycle.BackupJobTransitions},
		{"RESTORE_TRANSITIONS", lifecycle.RestoreTransitions},
		{"SCHEDULED_JOB_TRANSITIONS", lifecycle.ScheduledJobTransitions},
	} {
		encoded, _ := json.Marshal(group.values)
		output.WriteString("export const " + group.name + " = ")
