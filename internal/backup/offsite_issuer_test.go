...
func testOneRunEndpoint(t *testing.T) (*OneRunEndpoint, adapter.SessionRequest, []byte) {
	...
	request := adapter.SessionRequest{RunID: "run-a", StepID: "step-a", PointID: "point-a", GenerationID: "generation-a", RecoveryEpoch: 7, Deadline: now.Add(5 * time.Minute), TTL: time.Minute, Prefix: "critical/generation-a/", Actions: []string{"ListBucket", "PutObject"}}
...
func (fixture custodyFixture) RunOffsiteRestic(_ context.Context, request OffsiteResticRequest) (OffsiteResticResult, error) {
	...
	return OffsiteResticResult{RepositoryID: "offsite-generation-a", SnapshotID: fixture.point.SnapshotID, InventoryDigest: fixture.point.InventoryDigest, ObjectCount: fixture.point.ObjectCount, ObjectBytes: fixture.point.ObjectBytes, IAMCalled: true, ChildExited: true}, nil
}
...
func TestCopyOffsitePointUsesCustodyAndOneRunIAM(t *testing.T) {
	...
	admission := GenerationAdmission{GenerationID: "generation-a", Prefix: "critical/generation-a/", RuleDigest: offsiteDigest("f")}
	...
	if err != nil || pending.PointID != point.PointID || len(pending.SessionExpiries) != 1 {
