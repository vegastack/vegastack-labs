...
func SelectLocalRetirement(catalog []RetirementCandidate, lastGoodIDs []string, window time.Duration, epoch int64) (RetirementSelection, error) {
	...
	for _, candidate := range sorted {
		if candidate.PointID == "" || candidate.SnapshotID == "" || candidate.RepositoryID == "" || candidate.RepositoryID != repo || candidate.RecoveryEpoch != epoch || candidate.Bytes < 0 || candidate.CreatedAt.IsZero() || !validBackupManifestDigest(candidate.ManifestDigest) || !validBackupManifestDigest(candidate.DependencyDigest) || !validBackupManifestDigest(candidate.InventoryDigest) || (candidate.ProofDigest != "" && !validBackupManifestDigest(candidate.ProofDigest)) || snapshots[candidate.SnapshotID] {
			return invalid()
		}
