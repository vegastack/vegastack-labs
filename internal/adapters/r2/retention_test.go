...
func TestRetentionClientAcceptsCloudflareEnvelopeAndBindsObservedRuleCount(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_, _ = writer.Write([]byte(`{"success":true,"errors":[],"messages":[],"result":{"rules":[` +
			...
			`{"id":"other-a","prefix":"critical/gen-b/config","enabled":true,"condition":{"type":"Indefinite"}},` +
			`{"id":"other-b","prefix":"critical/gen-b/keys/","enabled":true,"condition":{"type":"Indefinite"}},` +
			`{"id":"other-c","prefix":"critical/gen-b/data/","enabled":true,"condition":{"type":"Indefinite"}},` +
			`{"id":"other-d","prefix":"critical/gen-b/index/","enabled":true,"condition":{"type":"Indefinite"}},` +
			`{"id":"other-e","prefix":"critical/gen-b/snapshots/","enabled":true,"condition":{"type":"Indefinite"}}]}}`))
	}))
	defer server.Close()
	...
	observation, err := (RetentionClient{APIBase: server.URL, AccountID: "account-a", Bucket: "bucket-a", Client: server.Client(), Clock: func() time.Time { return now },
		AvailableBytes: 100, AvailablePUTs: 10, AvailableLISTs: 3, RuleCount: 10, RuleLimit: 1000, RetainedGenerations: 2}).Observe(context.Background(), "gen-a", "critical/gen-a", []byte("observer-secret"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, rule := range observation.ProtectedRules {
		if strings.Contains(rule.Prefix, "gen-b") {
			t.Fatalf("other generation rule leaked into observation: %+v", rule)
		}
	}
}
