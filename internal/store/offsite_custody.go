...
func (repository *OffsiteRepository) BeginCustody(ctx context.Context, binding OffsiteCustodyBinding) error {
	...
	return repository.inCustodyTx(ctx, func(ctx context.Context, transaction *sql.Tx) error {
		_, err := transaction.ExecContext(ctx, `INSERT INTO backup_offsite_custody_attempts(attempt_id,plan_id,plan_digest,run_id,step_id,lease_id,generation_id,source_point_id,state_revision,recovery_epoch,maximum_expires_at,nonce_digest,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`,
			binding.AttemptID, binding.PlanID, binding.PlanDigest, binding.RunID, binding.StepID, binding.LeaseID, binding.GenerationID, binding.SourcePointID, binding.StateRevision, binding.RecoveryEpoch, binding.MaximumExpiresAt.UTC().Format(time.RFC3339), binding.NonceDigest, createdAt)
		return err
	})
}
