...
func (repository *OffsiteRepository) AppendRunSpec(ctx context.Context, record OffsiteRunSpecRecord) error {
	if repository == nil || repository.backup == nil || repository.backup.store == nil || record.GenerationID == "" || record.SourcePointID == "" || record.SnapshotPath == "" || record.RepositoryURL == "" ||
		record.ParentReferenceID == "" || record.RepositoryKeyReferenceID == "" || record.ObserverReferenceID == "" || record.ParentReferenceID == record.RepositoryKeyReferenceID || record.ParentReferenceID == record.ObserverReferenceID || record.RepositoryKeyReferenceID == record.ObserverReferenceID || record.RuleDigest == "" || record.G008EvidenceDigest == "" || len(record.CanonicalJSON) < 2 ||
		record.MaximumBytes <= 0 || record.MaximumPUTs <= 0 || record.MaximumLISTs <= 0 || record.MaximumRetainedGenerations <= 0 || record.RuleLimit <= 0 || record.RetentionSeconds <= 0 || record.SessionTTLSeconds <= 0 || record.SessionTTLSeconds > 900 || record.SourceRevision < 0 || record.StateRevision < 0 || record.RecoveryEpoch < 0 {
	...
	return repository.backup.inTx(ctx, func(ctx context.Context, tx *sql.Tx) error {
		...
		_, err := tx.ExecContext(ctx, `INSERT INTO backup_offsite_run_specs(generation_id,source_point_id,snapshot_path,repository_url,parent_reference_id,repository_key_reference_id,observer_reference_id,rule_digest,g008_evidence_digest,maximum_bytes,maximum_puts,maximum_lists,maximum_retained_generations,rule_limit,retention_seconds,session_ttl_seconds,canonical_json,source_revision,state_revision,recovery_epoch,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
			record.GenerationID, record.SourcePointID, record.SnapshotPath, record.RepositoryURL, record.ParentReferenceID, record.RepositoryKeyReferenceID, record.ObserverReferenceID, record.RuleDigest, record.G008EvidenceDigest, record.MaximumBytes, record.MaximumPUTs, record.MaximumLISTs, record.MaximumRetainedGenerations, record.RuleLimit, record.RetentionSeconds, record.SessionTTLSeconds, string(record.CanonicalJSON), record.SourceRevision, record.StateRevision, record.RecoveryEpoch, createdAt)
		return backupWriteError(err)
	})
}
...
func (repository *OffsiteRepository) RunSpec(ctx context.Context, generationID string) (OffsiteRunSpecRecord, error) {
	...
	err := repository.backup.store.Read(ctx, func(tx ReadTx) error {
		return tx.queryRow(ctx, `SELECT generation_id,source_point_id,snapshot_path,repository_url,parent_reference_id,repository_key_reference_id,observer_reference_id,rule_digest,g008_evidence_digest,maximum_bytes,maximum_puts,maximum_lists,maximum_retained_generations,rule_limit,retention_seconds,session_ttl_seconds,canonical_json,source_revision,state_revision,recovery_epoch FROM backup_offsite_run_specs WHERE generation_id=?`, generationID).
			Scan(&record.GenerationID, &record.SourcePointID, &record.SnapshotPath, &record.RepositoryURL, &record.ParentReferenceID, &record.RepositoryKeyReferenceID, &record.ObserverReferenceID, &record.RuleDigest, &record.G008EvidenceDigest, &record.MaximumBytes, &record.MaximumPUTs, &record.MaximumLISTs, &record.MaximumRetainedGenerations, &record.RuleLimit, &record.RetentionSeconds, &record.SessionTTLSeconds, &canonical, &record.SourceRevision, &record.StateRevision, &record.RecoveryEpoch)
	})
