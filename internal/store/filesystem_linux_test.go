...
func TestLocalFilesystemTypesFailClosed(t *testing.T) {
	for _, filesystemType := range []uint64{0xef53, 0x58465342, 0x9123683e, 0xf2f52010, 0x2fc12fc1} {
		if !isLocalFilesystemType(filesystemType) {
			t.Fatalf("known local filesystem %x rejected", filesystemType)
		}
	}
	for _, filesystemType := range []uint64{0x6969, 0x517b, 0xff534d42, 0x65735546, 0x5346414f, 0x01021997, 0x00c36400, 0x794c7630, 0x01021994, 0} {
		if isLocalFilesystemType(filesystemType) {
