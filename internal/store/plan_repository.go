	...
	intent, err := repository.store.writeIntent(ctx, intentRequest{Expected: &request.Expected, Idempotency: key, Event: event}, func(ctx context.Context, transaction *sql.Tx) error {
		...
		if request.DesiredDeclaration.DeclarationType == "backup.offsite" {
			...
			if _, insertErr := transaction.ExecContext(ctx, `INSERT INTO backup_offsite_run_specs(generation_id,source_point_id,snapshot_path,repository_url,parent_reference_id,repository_key_reference_id,observer_reference_id,rule_digest,g008_evidence_digest,maximum_bytes,maximum_puts,maximum_lists,maximum_retained_generations,rule_limit,retention_seconds,session_ttl_seconds,canonical_json,source_revision,state_revision,recovery_epoch,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
				spec.GenerationID, spec.SourcePointID, spec.SnapshotPath, spec.RepositoryURL, spec.ParentReferenceID, spec.RepositoryKeyReferenceID, spec.ObserverReferenceID, spec.RuleDigest, spec.G008EvidenceDigest, spec.MaximumBytes, spec.MaximumPUTs, spec.MaximumLISTs, spec.MaximumRetainedGenerations, spec.RuleLimit, spec.RetentionSeconds, spec.SessionTTLSeconds, string(canonicalSpec), spec.SourceRevision, request.Plan.Binding.StateRevision, request.Plan.Binding.RecoveryEpoch, request.DesiredDeclaration.CreatedAt); insertErr != nil {
