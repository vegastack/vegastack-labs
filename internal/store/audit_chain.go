...
func (store *Store) appendAuditLink(ctx context.Context, tx *sql.Tx, event audit.Event, ids audit.ContextIDs) (audit.ChainLink, error) {
	...
	previous, sequence, err := ensureAuditGenesis(ctx, tx, instanceID, event.RecoveryEpoch)
	if err != nil {
		return audit.ChainLink{}, store.transactionError(ctx, err)
	}
...
func ensureAuditGenesis(ctx context.Context, tx *sql.Tx, instanceID string, epoch int64) (audit.Fingerprint, int64, error) {
...
func (store *Store) ChainRange(ctx context.Context, first, last audit.EventID) (audit.ChainRange, error) {
	...
	for rows.Next() {
		...
		if err := json.Unmarshal(contextBytes, &struct {
			Domain string `json:"domain"`
			*audit.ContextIDs
		}{ContextIDs: &link.Context}); err != nil {
