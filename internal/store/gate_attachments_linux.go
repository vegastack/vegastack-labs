...
// This restricted digest-addressed primitive is deliberately not wired to
// public evidence intake or a production collector in #104. It confers no
// live-proof authority by itself; any future caller needs a separately
// reviewed collector, subject binding and proof verifier.
...
func NewGateAttachmentStore(root string, expectedUID uint32) (*GateAttachmentStore, error) {
	...
	if err := unix.Close(fd); err != nil {
		return nil, newStoreError(generated.ErrorCodeIntegrityFailure, "gate-attachment-directory", false, nil)
	}
	return store, nil
}
...
func (store *GateAttachmentStore) openRoot() (int, error) {
	...
	if err != nil {
		return -1, newStoreError(generated.ErrorCodeIntegrityFailure, "gate-attachment-directory", false, err)
	}
...
func gateAttachmentName(digest string) (string, error) {
	if !gateAttachmentDigestPattern.MatchString(digest) {
		return "", newStoreError(generated.ErrorCodeInputInvalid, "gate-attachment-digest", false, nil)
	}
...
func (store *GateAttachmentStore) Put(ctx context.Context, digest string, data []byte) error {
	if err := ctx.Err(); err != nil {
		return newStoreError(generated.ErrorCodeInterrupted, "gate-attachment", false, err)
	}
...
func (store *GateAttachmentStore) Get(ctx context.Context, digest string) ([]byte, error) {
	if err := ctx.Err(); err != nil {
		return nil, newStoreError(generated.ErrorCodeInterrupted, "gate-attachment", false, err)
	}
	name, err := gateAttachmentName(digest)
	if err != nil {
		return nil, err
	...
	if err != nil || gateDigest(raw) != digest {
		return nil, newStoreError(generated.ErrorCodeIntegrityFailure, "gate-attachment", false, err)
	}
	return raw, nil
}
...
func (store *GateAttachmentStore) readAt(root int, name string) ([]byte, error) {
	...
	if err != nil || len(raw) > gateAttachmentMaxBytes {
		return nil, unix.EIO
	}
	return raw, nil
}
