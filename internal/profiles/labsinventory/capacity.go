...
func mapHardwareRow(record int, row []string, assetID inventory.LocalID, capturedAt time.Time) ([]inventory.DraftObservation, []inventory.DraftHardwareFact, []inventory.FieldProvenance, []inventory.Finding) {
	observations := []inventory.DraftObservation{
		{ID: localID(record, "factory-observation"), SubjectID: assetID, Kind: "labs-sheet1-factory", Value: "reported", ObservedAt: capturedAt},
		{ID: localID(record, "current-observation"), SubjectID: assetID, Kind: "labs-sheet1-current", Value: "preferred", ObservedAt: capturedAt},
	}
	...
	for _, field := range textHardwareFields {
		...
		copy := value
		facts = append(facts, inventory.DraftHardwareFact{ID: localID(record, field.suffix), Kind: field.kind, TextValue: &copy})
	}
	for _, field := range integerHardwareFields {
		...
		copy := parsed
		facts = append(facts, inventory.DraftHardwareFact{ID: localID(record, field.suffix), Kind: field.kind, IntegerValue: &copy, Unit: field.unit})
	}
