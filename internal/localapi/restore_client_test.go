...
func TestRestoreClientUsesRegisteredResourceRoutes(t *testing.T) {
	plan, run, verify := restoreClientRequests(t)
	tests := []struct {
		name string
		path string
		call func(Client, generatedRestoreProfile)
	}{
		{name: "plan", path: "/api/v1/recovery-points/point-a/restore-plans", call: func(client Client, fixture generatedRestoreProfile) {
			_, _ = client.PlanRestore(context.Background(), fixture.profile, plan)
		}},
		{name: "run", path: "/api/v1/restore-plans/plan-a/runs", call: func(client Client, fixture generatedRestoreProfile) {
			_, _ = client.RunRestore(context.Background(), fixture.profile, run)
		}},
		{name: "verify", path: "/api/v1/restore-plans/plan-a/verifications", call: func(client Client, fixture generatedRestoreProfile) {
			_, _ = client.VerifyRestore(context.Background(), fixture.profile, verify)
		}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, profile, captured := serveFixedResponse(t, http.StatusOK, []byte("{}\n"))
			test.call(NewClient(clientTestFactory()), generatedRestoreProfile{profile: profile})
			request := <-captured
			if request.method != http.MethodPost || request.path != test.path {
				t.Fatalf("request=%#v want POST %s", request, test.path)
			}
		})
	}
}

// The alias keeps the route table compact without weakening the concrete
// server profile type used by the client boundary.
type generatedRestoreProfile struct{ profile serverconfig.Profile }

func restoreClientRequests(t *testing.T) (generated.RestoreRequest, generated.RestoreRunRequest, generated.RestoreVerifyRequest) {
	t.Helper()
