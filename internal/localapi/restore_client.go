...
func (client *client) PlanRestore(ctx context.Context, profile serverconfig.Profile, input generated.RestoreRequest) (TypedResponse[generated.RestoreBinding], error) {
	if !validPathToken(input.PointID) {
		return TypedResponse[generated.RestoreBinding]{}, failure.New(generated.ErrorCodeInputInvalid, "recovery-point", false)
	}
	return restoreMutation(client, ctx, profile, "/api/v1/recovery-points/"+input.PointID+"/restore-plans", "api.v1.restores.plan", generated.SchemaIDRestoreRequest, input, input.PointID, "")
}
...
func (client *client) RunRestore(ctx context.Context, profile serverconfig.Profile, input generated.RestoreRunRequest) (TypedResponse[generated.RestoreBinding], error) {
	...
	return restoreMutation(client, ctx, profile, "/api/v1/restore-plans/"+input.PlanID+"/runs", "api.v1.restores.run", generated.SchemaIDRestoreRunRequest, input, input.PointID, input.PlanID)
}
...
func (client *client) VerifyRestore(ctx context.Context, profile serverconfig.Profile, input generated.RestoreVerifyRequest) (TypedResponse[generated.RestoreVerification], error) {
	...
	return requestTyped(client, ctx, profile, requestSpec{localtransport.MethodPost, "/api/v1/restore-plans/" + input.PlanID + "/verifications", "api.v1.restores.verify", maxOperationResponseBodyBytes, operationTimeout, true}, input, func(data generated.RestoreVerification, result generated.RunResult) bool {
		encoded, err := json.Marshal(data)
