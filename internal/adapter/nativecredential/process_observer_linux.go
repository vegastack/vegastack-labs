...
func (o invocationObserver) observe(ctx context.Context, binding credentialref.LifecycleBinding, reader credentialref.NativeConsumerBinding) (NativeInvocationProof, error) {
	if ctx == nil || ctx.Err() != nil || o.units == nil || o.authority == nil || o.inspect == nil || o.process == nil ||
		...
		return NativeInvocationProof{}, fmt.Errorf("%w: input", errNativeInvocation)
	}
	...
	if err != nil || before.State != "present" || before.Fingerprint != binding.CiphertextFingerprint || before.Device == 0 || before.Inode == 0 {
		return NativeInvocationProof{}, fmt.Errorf("%w: source-before", errNativeInvocation)
	}
	...
	if err != nil || receipt.UnitName != reader.UnitName || receipt.BootID != first.BootID || receipt.Status != "completed" ||
		...
		return NativeInvocationProof{}, fmt.Errorf("%w: restart", errNativeInvocation)
	}
	...
	if err != nil || validateAppliedSource(after, binding, reader, path) != nil || after.MachineID != first.MachineID || after.BootID != receipt.BootID ||
		...
		return NativeInvocationProof{}, fmt.Errorf("%w: applied-after", errNativeInvocation)
	}
	if !unitIdentityMatches(after, reader) {
		return NativeInvocationProof{}, fmt.Errorf("%w: service-identity", errNativeInvocation)
	}
	process, err := o.process(ctx, after, reader)
	if err != nil || process.MainPID != after.MainPID || process.StartTicks == 0 || process.UID != reader.ServiceUID || process.GID != reader.ServiceGID {
		return NativeInvocationProof{}, fmt.Errorf("%w: process-identity", errNativeInvocation)
	}
	...
	if err != nil || !validProbeResult(loaded) || loaded.Status != AccessProbeOpened ||
		...
		loaded.Mode&unix.S_IFMT != unix.S_IFREG || loaded.Mode&0o022 != 0 || loaded.NamespaceInode == 0 {
		return NativeInvocationProof{}, fmt.Errorf("%w: positive-open", errNativeInvocation)
	}
	...
	if err != nil || !sameInvocation(after, rechecked) || validateAppliedSource(rechecked, binding, reader, path) != nil {
		return NativeInvocationProof{}, fmt.Errorf("%w: applied-recheck", errNativeInvocation)
	}
	processAgain, err := o.process(ctx, rechecked, reader)
	if err != nil || processAgain != process {
		return NativeInvocationProof{}, fmt.Errorf("%w: process-recheck", errNativeInvocation)
	}
	loadedAgain, err := o.authority.Probe(ctx, request)
	if err != nil || loadedAgain != loaded {
		return NativeInvocationProof{}, fmt.Errorf("%w: loaded-recheck", errNativeInvocation)
	}
	...
	if err != nil || last != before || ctx.Err() != nil {
		return NativeInvocationProof{}, fmt.Errorf("%w: source-recheck", errNativeInvocation)
	}
	return NativeInvocationProof{BootID: after.BootID, InvocationID: after.InvocationID, MainPID: after.MainPID, ProcessStartTicks: process.StartTicks,
		NamespaceDevice: loaded.NamespaceDevice, NamespaceInode: loaded.NamespaceInode,
...
func observeProcessIdentity(ctx context.Context, snapshot AppliedUnitSnapshot, reader credentialref.NativeConsumerBinding) (ProcessIdentity, error) {
	if ctx == nil || ctx.Err() != nil || snapshot.MainPID <= 1 || snapshot.MainPID > 0x7fffffff || !unitIdentityMatches(snapshot, reader) {
		return ProcessIdentity{}, fmt.Errorf("%w: process-input", errNativeInvocation)
	}
	path := fmt.Sprintf("/proc/%d", snapshot.MainPID)
	stat, err := os.ReadFile(path + "/stat")
	if err != nil {
		return ProcessIdentity{}, fmt.Errorf("%w: proc-stat", errNativeInvocation)
	}
	end := bytes.LastIndexByte(stat, ')')
	if end < 0 || end+2 >= len(stat) {
		return ProcessIdentity{}, fmt.Errorf("%w: proc-stat-shape", errNativeInvocation)
	}
	fields := strings.Fields(string(stat[end+2:]))
	if len(fields) <= 19 {
		return ProcessIdentity{}, fmt.Errorf("%w: proc-stat-fields", errNativeInvocation)
	}
	start, err := strconv.ParseUint(fields[19], 10, 64)
	if err != nil || start == 0 {
		return ProcessIdentity{}, fmt.Errorf("%w: proc-start", errNativeInvocation)
	}
	status, err := os.ReadFile(path + "/status")
	if err != nil || !procStatusIdentityMatches(string(status), reader.ServiceUID, reader.ServiceGID) {
		return ProcessIdentity{}, fmt.Errorf("%w: proc-uid-gid", errNativeInvocation)
	}
	request := AccessProbeRequest{UID: reader.ServiceUID, GID: reader.ServiceGID, UnitName: reader.UnitName, CredentialName: reader.LoadedName,
	...
	if verifyTargetProcess(request) != nil || ctx.Err() != nil {
		return ProcessIdentity{}, fmt.Errorf("%w: proc-unit", errNativeInvocation)
	}
	return ProcessIdentity{MainPID: snapshot.MainPID, StartTicks: start, UID: reader.ServiceUID, GID: reader.ServiceGID}, nil
}
