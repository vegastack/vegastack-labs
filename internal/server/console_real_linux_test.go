...
func TestEmbeddedConsoleAndAuthorizedAPIShareOriginWhileLocalRecoverySurvives(t *testing.T) {
	...
	application, err := api.NewApplication(api.Config{Authority: authority, Authorizer: store.NewReadAuthorizer(authority), Reads: store.NewReadRepository(authority), Results: factory, Sessions: authenticator.SessionService()})
	if err != nil {
		t.Fatal(err)
	}
	...
	profile := serverconfig.Profile{
		...
		ShutdownGrace: 5 * time.Second, PrincipalBindings: []identity.Binding{{UID: uint32(os.Getuid()), PrincipalID: "principal.local"}}, RemoteRead: serverconfig.RemoteRead{Enabled: true},
	}
	...
	response, err = client.Do(consoleStackRequest(t, http.MethodGet, "https://"+address+"/api/v1/summary", assertion, session))
	if err != nil {
		t.Fatal(err)
	}
	...
	local, err := localClient.Status(context.Background(), profile)
	if err != nil || local.Status.RemoteReadState != "ready" || !local.Status.ReadAvailable {
		t.Fatalf("local recovery status = %#v, %v", local.Status, err)
	}
	cancel()
	if err := <-done; err != nil {
		t.Fatal(err)
