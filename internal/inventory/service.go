...
func NewService(repository DraftRepository, generator IDGenerator) (*Service, error) {
	if repository == nil {
	...
	return &Service{repository: repository, newID: generator}, nil
}
...
func (service *Service) ValidateAndStore(ctx context.Context, request ImportRequest) (ImportResult, error) {
	if service == nil || service.repository == nil || len(request.IdempotencyKey) == 0 || len(request.IdempotencyKey) > MaxIdempotencyBytes || !utf8.ValidString(request.IdempotencyKey) {
	...
	put, err := service.repository.Put(ctx, PutDraftRequest{ExpectedStateRevision: request.ExpectedStateRevision, IdempotencyKeyDigest: "sha256:" + hex.EncodeToString(keyDigest[:]), CandidateDigest: normalized.ContentDigest, DraftID: draftID, Draft: normalized})
	if err != nil {
		return ImportResult{}, sanitizeRepositoryError(err)
	}
	return ImportResult{DraftID: put.Ref.ID, DraftRevision: put.Ref.Revision, ValidationStatus: normalized.ValidationStatus, SourceDigest: normalized.Candidate.Source.Digest, ContentDigest: normalized.ContentDigest, StateRevision: put.CommitStateRevision, RecoveryEpoch: put.RecoveryEpoch, Created: put.Created, Counts: normalized.Counts, Findings: slices.Clone(normalized.Findings)}, nil
}
