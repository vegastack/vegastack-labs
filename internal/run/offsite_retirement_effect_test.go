...
func (f *retirementExecutionFixture) RetireOffsite(context.Context, adapter.Operation, adapter.ExactExecutionBinding, *credentialref.Value, *credentialref.Value) (string, error) {
	f.calls++
	return f.digest, nil
}
...
func TestOffsiteRetirementEffectRequiresExactBoundHumanPlan(t *testing.T) {
	...
	result, err := effect.ExecuteBoundWithCredentials(context.Background(), op, binding, []*credentialref.Value{lock, retention})
	...
	if _, err := effect.ExecuteBoundWithCredentials(context.Background(), op, binding, []*credentialref.Value{lock, retention}); err == nil {
