...
func phase4Schemas() []SchemaDefinition {
	...
	receiptFields := append(leaseReceiptBinding(id, digest, nonnegative), id("receiptId", "ReceiptID"), FieldDefinition{JSONName: "status", GoName: "Status", Kind: ValueString, Required: true, Enum: []string{"failed", "partial", "running", "succeeded"}}, digest("resultDigest", "ResultDigest"), nullableID("pendingPointId", "PendingPointID"), timestamp("recordedAt", "RecordedAt"), extensions)
