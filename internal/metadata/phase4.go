...
func phase4Schemas() []SchemaDefinition {
	...
	nullableDigest := func(name, goName string) FieldDefinition {
		return FieldDefinition{JSONName: name, GoName: goName, Kind: ValueString, Required: true, Nullable: true, Pattern: `^sha256:[a-f0-9]{64}$`}
	}
	...
	leaseFields := append(leaseReceiptBinding(id, digest, nonnegative), timestamp("claimedAt", "ClaimedAt"), timestamp("renewAfter", "RenewAfter"), timestamp("leaseExpiresAt", "LeaseExpiresAt"), timestamp("maximumExpiresAt", "MaximumExpiresAt"), FieldDefinition{JSONName: "status", GoName: "Status", Kind: ValueString, Required: true, Enum: []string{"active", "expired", "released", "revoked"}}, extensions)
