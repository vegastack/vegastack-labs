...
func validateCommands(commands []CommandDefinition, schemas map[string]struct{}) error {
	...
	for index, command := range commands {
		...
		case AvailabilityAvailable:
			...
			case "apply", "run cancel", "run resume", "gate evidence", "gate profile draft", "credential import", "backup policy draft", "backup retention-locks draft", "backup retirement draft", "backup run", "backup verify", "credential stage", "credential activate", "credential rotate", "credential revoke", "credential recover", "recovery witness collect":
